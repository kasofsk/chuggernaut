...
#
# Why a gate and not just the one fix (job #501): the divergence is invisible to
...
# happens to follow — in build-worker.sh a later apostrophe closed the run, so
...
# other shell: `deploy/prod/build-worker.sh` is run by `deploy/prod/update.sh` on
# the Mini and by operators from their laptops, both macOS, where /bin/sh IS
# bash. The instance this gate was written for silently moved a whole host-mode
# pre-flight inside the `if` above it, so the guard ran only when its own check
# FAILED and the passing shape died on an unbound variable.
#

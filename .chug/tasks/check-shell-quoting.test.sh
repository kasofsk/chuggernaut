...
#      deploy/prod/build-worker.sh before job #501 fixed it — an apostrophe in a
...
# depends on what follows it (in build-worker.sh a later apostrophe closed the
...
# 1. The instance. Reproduced as build-worker.sh carried it: the expansion sits
...
# The premise: a POSIX shell accepts the fixture outright, and bash reads it as
# something else. If this ever stops holding, the gate guards nothing.
/bin/sh -n "$WORK/broken.sh" 2> /dev/null \
	&& echo "ok   - /bin/sh -n accepts the broken fixture (no POSIX syntax check catches this)" \
	&& pass=$((pass + 1)) \
	|| { echo "FAIL - /bin/sh -n rejected the fixture; the premise no longer holds"; fail=$((fail + 1)); }

disagree() { # <name> <fixture> <want: yes|no>
	if [ "$(/bin/sh "$2" 2>&1)" = "$(bash "$2" 2>&1)" ]; then got=no; else got=yes; fi
	if [ "$got" = "$3" ]; then
		echo "ok   - $1"
		pass=$((pass + 1))
	else
		echo "FAIL - $1: /bin/sh and bash disagree=$got, wanted $3"
		fail=$((fail + 1))
	fi
}

if command -v bash > /dev/null 2>&1; then
	disagree "/bin/sh and bash disagree about the flagged fixture, which is the whole class" "$WORK/broken.sh" yes
	...
	echo "skip - no bash on PATH, so the disagreements went unmeasured"
fi

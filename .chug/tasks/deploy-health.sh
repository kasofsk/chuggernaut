#!/bin/sh
# Deploy health gate — the *real* evaluator for a `deploy` job (.chug/jobs/deploy.yaml).
#
# A deploy job carries no commits, so the appended project ci default
# (.chug/jobs/_defaults.yaml) has an empty diff and glob-skips its cargo work in
...
# declared secret on this evaluator (.chug/jobs/deploy.yaml; provision per deploy/prod

...
1. **Front-end only.** This job type exists so UI changes ship without the
   ...
   outside `web/` (and, only when the brief explicitly says so, `.chug/jobs/` or
   `.chug/prompts/`). If the brief genuinely requires an API change, stop: call
...
3. Match the existing component and CSS style; keep the change minimal.
   **Write no comments.** Source carries doc comments only (`/** … */`), and a
   ...
   CI on any non-doc comment left anywhere in the tree. What a comment would
   have said belongs in a doc — work `.chug/tasks/docs-update.md` before you
   submit, and name the docs you updated in your summary — or, if it is

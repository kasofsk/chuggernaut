...
| `decide/work` | The C6 Work-phase decider: the launch-time validation fork (a job still `Ready` has no work task, so every pre-work park is `Stalled` — `config_schema_skew` for §14.2 skew, `launch_validation_failed` otherwise; only a rework re-entry escalates), one attempt's task record incl. claim parking and claim consumption, the exit verdict with the finish-line guard, and the one retry policy every Work failure spends. | §3.2, §1.2 |
...
| `knowledge` | §4.4 upfront injection, pure half: what a `knowledge:` entry names — a repo page by path, a bare name's config-root tag file — the ordered deduplicated union of type defaults and job tags, and the `## Project Knowledge` block they compose into. | §4.4 |
| `release` | Release validation, ref-reading half: `.chug/jobs/*.yaml` loading + prompt/KV checks through the `vcs` port, plus the merge-time skew scan of a branch's config; re-exports the pure half. | §2.2, §14 |

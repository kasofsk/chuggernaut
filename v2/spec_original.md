...
eval_retries: int              # optional; default 1; retry budget for agent-type evaluators only (command and human evaluators are never retried)
rework_budget: int              # optional; default 0; disallowed for command work; number of rework cycles permitted after the initial cycle (0 = no rework)
...
| `eval_retries` (top-level; applies to agent-type evaluators only; command and human evaluators are never retried) | optional | optional | optional |
| `rework_budget` (top-level) | optional | disallowed | optional |
...
`work_retries` is disallowed — there is no container to retry. If eval fails and the rework budget has not been exhausted, the job re-enters Work and a new Human task is created for the next cycle, with all eval findings from the previous cycle injected into the task prompt so the operator knows what to fix. Human work tasks are excluded from the timeout scan (`task_timeout` has no effect); use `job_deadline` to bound total wall-clock time if needed.
...
rework_budget: 2     # max rework cycles after the initial; rework_budget: 2 → cycles 1, 2, 3 permitted; failure at end of cycle 3 → escalation
inputs:
...
pub struct Job {
    ...
    pub base_ref: Option<String>,          // exact HEAD of default branch at Ready-transition; None until then
...
Example record (JSON representation in NATS KV):
...
Named inputs are dependency declarations only — they establish DAG edges and ordering, not data routing.

**Validation happens in three passes, each at the right moment:**
1. **Release-time** — input wiring + static file existence + secrets/vars existence in KV, checked against current HEAD. Fast feedback before any execution is committed. Not an execution guarantee — files and KV entries can change before the job runs.
2. **Ready-transition** — static file existence re-checked at `base_ref` (the exact HEAD locked when the job enters Ready). Eliminates TOCTOU drift between approval and execution; all subsequent execution uses this ref exclusively. Secrets/vars are not re-checked here — their values are live KV entries, not pinned to base_ref.
...
Release validation runs on every release request — both per-job (`POST .../jobs/{seq}/release`) and graph-level (`POST .../graph/validate`, `POST .../graph/release`). The dispatcher enforces all of the following before any job leaves Frozen:
...
Frozen → Ready | Blocked → Work → Evaluation → Done
                              ↑         |
                              └─────────┘  (rework: eval failed under rework budget; or merge conflict — cycle++ but rework_budget not consumed)
Work → Escalated                          (work retries exhausted)
Evaluation → Escalated                    (rework budget exhausted)
...
- **Escalated** — human intervention required; triggered automatically by work retries exhausted or rework budget exceeded; operator resolves via task inbox
...
`rework_budget` is the maximum number of rework cycles permitted after the initial one. With `rework_budget: 2`, cycles 1, 2, and 3 are permitted; eval failure at the end of cycle 3 triggers escalation. Equivalently: escalation occurs when `current_cycle > rework_budget + 1`.
...
pub struct TokenUsage {
    ...
    pub cache_read_tokens: Option<u64>,    // prompt cache reads (provider-specific; None if not applicable)
    pub cache_write_tokens: Option<u64>,   // prompt cache writes (provider-specific; None if not applicable)
}

pub enum TaskResolution {
    Pass       { structured: Option<serde_json::Value> },
    Fail       { structured: serde_json::Value },              // structured required on fail; on a regular Human task (work or eval phase) this creates an escalation Human task and transitions the job to Escalated
    Escalation { action: EscalationAction, structured: Option<serde_json::Value> },  // only valid on escalation Human tasks; drives the next transition via action
}
```

**Token usage**: agent work tasks (`TaskResult::Work`) and agent eval tasks (`TaskResult::Agent`) carry an optional `token_usage` field populated at submit time. The agent process accumulates token counts from LLM API responses throughout its run and includes the totals in its `submit_result` or `submit_eval` call. `token_usage` is `None` when a container crashes before submitting (infra error) — this is expected and does not affect retry logic. Command and human tasks never carry token usage.

`action` is only set on Human tasks created for escalation (work retries exhausted or rework budget exceeded). It is ignored on Human tasks in the normal Evaluation phase.
...
| Agent eval task: container exits (any code) with no prior `submit_eval`, `eval_retries` available | Infra error; new task record (same cycle, attempt++); other eval tasks continue in parallel |
...
| Eval reduce fails (`work.type: agent`), under rework budget | Job re-enters Work (cycle++); all eval results from this cycle feed into the next work task's rework context |
| Eval reduce fails (`work.type: agent`), rework budget exhausted | Human task: `{ prompt: "eval failed N cycles — decide" }` → job → Escalated |
| Eval reduce fails (`work.type: command`) | `rework_budget` is disallowed for command jobs; eval failure always escalates: Human task → job → Escalated |

`work_retries` counts container failures (crash, timeout, non-zero exit from an infra error) within a single Work phase. `eval_retries` counts the same for individual eval containers. `rework_budget` counts evaluation-driven rework cycles and is entirely separate from both.
...
- `Retry` — re-enters Work in the **same cycle** (no cycle increment; no rework budget consumed). The existing `job/{seq}` branch is used as-is — unlike automatic retries, it is **not** reset to `base_ref`, since the operator may have modified it. `work_retries` is not reset: if this attempt fails, the job escalates again immediately.
...
1. On `req.jobs.create.*`: write the job record to `jobs.*` KV, then publish `job-created` to the event stream. The `rdeps` index is updated after the job write; a write failure here is non-fatal and does not roll back the job write — the index is always rebuilt from `jobs.*` KV on startup and is never the source of truth for dependency resolution (see restart reconciliation)
...
6. On `req.vcs.*`: serve VCS queries (diff, tree, blob, log) by shelling out to `git` against the bare repo on disk; return results to the API layer
7. On restart: reconciliation pass — query Docker for each task in `Running` state and resume or resolve it (see Restart Reconciliation below). Any job with all dependencies Done but still Blocked is transitioned to Ready
...
12. On eval reduce pass: squash-merge `job/{seq}` to default branch. If no commits exist on `job/{seq}` beyond `base_ref`, this is a no-op — transition job directly to Done. If commits exist and merge is clean → transition job to Done. If conflict → update `base_ref` to current default HEAD, increment cycle (without consuming `rework_budget`), reset `job/{seq}` to the new `base_ref`, inject conflict context (conflicting files and description of upstream changes), and re-enter Work (step 2). No human task is created — the work agent will redo its changes on the updated base.
13. On eval reduce product failure: for `work.type: agent | human` jobs — if under rework budget, increment cycle, inject all eval findings into rework context, re-enter Work (step 2); if rework budget exhausted, create Human escalation task and transition to Escalated. For `work.type: command` jobs — `rework_budget` is disallowed, so eval failure always escalates immediately (no rework loop)
14. On escalation Human task completion: read `action` (see Escalation resolution in Tasks section for full semantics) — `Retry`: create new work task in the same cycle, execute per job type; `Resolve`: re-enter Evaluation (step 9); `Revoke`: transition to Revoked
...
**`agent`** — dispatcher invokes the configured `AgentProvider` with the eval prompt (path resolved from the repo). The provider launches a container from the evaluator's job `image`, which clones the repo on the job branch, inspects the diff, reasons about the criteria, and calls `submit_eval` to publish its verdict and structured findings to the dispatcher. The dispatcher writes `TaskResult::Agent` to `tasks.*` KV. **Verdict contract**: the sole way to record a product verdict is via `submit_eval` — the `pass` field in the payload is authoritative. Any container exit (zero or non-zero) without a prior `submit_eval` call is an infra error: the dispatcher retries up to `eval_retries`, then marks the task Failed (infra error). This is categorically distinct from a task that completed with `pass=false`.
...
- `Agent` — pass/fail set by the `pass` field in the `submit_eval` payload (product verdict). Any exit without a prior `submit_eval` = infra error, consumed by `eval_retries`. `structured` in `submit_eval` carries optional reasoning (e.g. `{ "notes": "..." }`).
...
On overall fail (`work.type: agent`): cycle++; `structured` from all eval results is collected into `AgentRunConfig.eval_context` and passed to the next work task. If `current_cycle > rework_budget + 1`: create Human escalation task, transition job to Escalated.

On overall fail (`work.type: command`): `rework_budget` is disallowed for command jobs; create Human escalation task immediately, transition job to Escalated. No rework loop.
...
| `reply` | work + eval agents | Write `AgentReply` to `channels.*` KV for display in the job detail view |
| `submit_result` | work agents | Publish work summary to dispatcher via `req.work.submit.*` request-reply; payload: `{ summary?, structured?, token_usage? }`; dispatcher writes task result and transitions to Evaluation |
| `submit_eval` | eval agents | Publish eval verdict and findings to dispatcher via `req.eval.submit.*` request-reply; payload must include `pass: bool`; optional fields: `structured`, `token_usage`; dispatcher writes `TaskResult::Agent` to `tasks.*` KV |
...
- **Agent eval containers** — `submit_eval` is required to record the product verdict. Any exit without a prior `submit_eval` is an infra error (see Evaluation section for full verdict contract).
...
Agent replies (`reply` tool) are written directly to the `channels.*` KV entry (same key as `update_status`), overwriting the previous reply. Reply history is not retained.
...
pub struct AgentRunConfig {
    ...
    pub eval_context: Vec<EvalResult>,     // empty on cycle 1 and merge-conflict cycles; populated on eval-failure rework cycles
    pub merge_conflict: Option<String>,    // set when cycle was triggered by a squash-merge conflict; describes conflicting files and upstream changes
}
...
# Token usage (aggregated from agent task records; command and human tasks excluded)
GET    /api/v1/projects/{owner}/{project}/usage                       → aggregate token usage across all agent tasks in the project
GET    /api/v1/projects/{owner}/{project}/jobs/{seq}/usage            → aggregate token usage for a single job (all cycles and attempts)
...
GET    /api/v1/projects/{owner}/{project}/vars/{name}
...
| `job-rework-started` | Dispatcher | Evaluation → Work with cycle++; includes new `cycle`, `reason` (`eval_failure` \| `merge_conflict`), and `eval_context` (populated for `eval_failure`; empty for `merge_conflict`) |
| `job-done` | Dispatcher | Evaluation → Done |
| `job-escalated` | Dispatcher | Work retries exhausted or rework budget exceeded; includes `reason` field |
...
| `task-completed` | Dispatcher | Task reached Done (includes `pass` and `structured` where applicable; includes `token_usage` for agent tasks when reported) |
...
Both usage endpoints return a `UsageSummary` aggregated from all agent task records (`TaskResult::Work` and `TaskResult::Agent`) that have a non-`None` `token_usage`. Tasks that crashed without submitting are excluded.
...
pub struct UsageSummary {
    ...
    pub task_count: u32,    // number of agent task attempts that reported usage
}
```

The dispatcher computes usage summaries on demand by scanning the relevant `tasks.*` KV entries — no separate aggregation store. This is acceptable because task records are small and execution throughput is agent-dominated (not high-frequency).
...
Direct git-over-SSH access for users is scoped by role. Member+ principals may push to any `job/{seq}` branch on their projects; the dispatcher controls branch integrity (hard-resetting on retry, squash-merging on completion). All other writes go through the dispatcher. Per-project read authorization is enforced against the user's `project_roles` claim in their SSH cert extension.

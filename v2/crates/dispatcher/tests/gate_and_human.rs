...
fn commit_and_move_main(rig: &Rig, file: &'static str) {
    ...
    rig.provider.on_run(move |cfg| async move {
        ...
        main.commit_file("docs/other.md", b"landed concurrently", "other job").await;
...
async fn merge_gate_reruns_ci_against_candidate_and_promotes() {
    ...
    assert!(m.read_file_at("acme", "api", "main", "docs/other.md").await.unwrap().is_some());
    assert!(m.read_file_at("acme", "api", "main", "src/a.rs").await.unwrap().is_some());
    ...
    assert_eq!(launches[1].env["JOB_BRANCH"], format!("merge-gate/{}", job.id));
    ...
    assert!(m.resolve_ref("acme", "api", &format!("merge-gate/{}", job.id)).await.is_err());
}
...
async fn merge_gate_failure_reworks_on_new_base_without_budget() {
    ...
    let bare = rig.repo.bare_path();
    rig.provider.on_run(move |cfg| async move {
        assert!(!cfg.eval_context.is_empty(), "gate findings must be injected");
        assert!(cfg.merge_conflict.is_some(), "conflict-style context must be injected");
        let branch = cfg.env.get("JOB_BRANCH").unwrap().clone();
        let clone = clone_branch_from(&bare, &branch).await;
        clone.commit_file("src/a.rs", b"job change v2", "implement again").await;
        clone.push(&branch).await;
    });
    ...
    let events = event_types(&rig.store).await;
    let reworks: Vec<&String> = events.iter().filter(|e| *e == "job-rework-started").collect();
    ...
    assert!(
        rig.repo.manager.read_file_at("acme", "api", "main", "src/a.rs").await.unwrap().is_some()
    );
}
...
async fn human_evaluator_and_human_work_resolve_via_inbox() {
    ...
    rig.handle.release_job("acme", "api", gated.id).await.unwrap();
    ...
    let err = rig
        .handle
        .resolve_task("acme", "api", gated.id, 2,
    ...
    rig.handle
        .resolve_task("acme", "api", gated.id, 2, TaskResolution::Pass { structured: None }, "david")
        .await
        .unwrap();
    ...
    rig.handle.release_job("acme", "api", manual.id).await.unwrap();
    wait_for_state(&rig.store, manual.id, JobState::Work).await;
    tokio::time::sleep(Duration::from_millis(100)).await;
    rig.handle
        .resolve_task("acme", "api", manual.id, 1, TaskResolution::Pass { structured: None }, "david")
        .await
        .unwrap();
    let done = wait_for_state(&rig.store, manual.id, JobState::Done).await;
    assert!(done.ready_at.is_some());

    let tasks = rig.store.tasks().await.unwrap().list_for_job("acme", "api", manual.id).await.unwrap();
...
async fn escalation_retry_relaunches_work_without_branch_reset() {
    ...
    rig.handle
        .resolve_task("acme", "api", job.id, 3,
            TaskResolution::Escalation { action: EscalationAction::Retry, structured: None },
            "david")
        .await
        .unwrap();

...
#
# Usage:
#   tasks/check-duplication.sh            # scan the repo root
#   tasks/check-duplication.sh <path>...  # scan only these paths (used by the
...
# The repo root is this script's parent, not the caller's cwd: the pre-commit
# hook, CI and a shell test all invoke it from different directories and must all
# scan (and configure from) the same tree. Resolved with shell builtins only, so
# the diagnostics below still work on a stripped PATH.
case "$0" in
*/*) here="${0%/*}" ;;
*) here="." ;;
esac
...
if [ "$status" -ne 0 ]; then
	...
	echo "!!!     threshold. Reproduce locally with: tasks/check-duplication.sh"
	exit 1
fi

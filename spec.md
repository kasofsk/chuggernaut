...
**Warm-target seeding (image property, not dispatcher machinery).** A build image MAY bake a prebuilt `target/` so containers start from "latest main already built" rather than compiling cold every task. This is entirely an **image concern** — like the node-local build cache (§3.1) it adds nothing to the launch message, the wire, or the dispatcher, and touches no per-job state. The `agent-rust` image (`deploy/prod/Dockerfile.agent-rust`) compiles the whole workspace at image-build time into `CARGO_TARGET_DIR=/opt/chug-prebuilt-target` (a path *outside* `/workspace`, baked as an image ENV), then deletes the build tree; the runtime clone into the empty `/workspace` reuses those artifacts from the image's read-only layer, and each container's writes land on its own copy-on-write overlay, so the seed is shared safely and copied never. Registry-dependency artifacts (checksum-keyed) reuse regardless of the clone's mtimes — the reliable win; workspace crates are mtime-sensitive and recompile against a fresh clone, a residual left to sccache (§3.1) and the warm dep graph. **A seed ships only the artifacts that are actually reused.** The same-layer prune that follows the build drops rustc's incremental state and the linked executables (workspace binaries, `--all-targets` test harnesses) — the units the mtime sensitivity above guarantees will be rebuilt anyway — keeping the dependency `.rlib`/`.rmeta` and build-script outputs. Baking them instead is dead weight in the image and, because a refresh holds a whole image generation twice while exporting it, dead weight in the node's disk pre-flight (§3.1). The prune must not change the build **profile**: a seed compiled differently from what agents run has different fingerprints, and cargo ignores it entirely. The seed is as fresh as the last image rebuild — the worker self-refresh on every deploy (§3.1) keeps it warm, so no extra staleness machinery is needed. It composes with the node-local cache: the seed warms the image's read-only layer, sccache reuses compiler invocations across jobs on the node.

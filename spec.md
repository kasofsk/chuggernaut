...
| `Draft` | `Frozen` | `POST .../jobs/{seq}/finalize` accepted | Job is Draft | Finalize the edited definition (validate field rules + evaluator collisions like release; wiring/static config deferred to release, as for a freshly-created Frozen job §2.1); park the job Frozen (re-batchable) instead of scheduling it; publish `job-finalized`. Validation failure rejects (422) and the job stays Draft |
...
**Dynamic worker registration.** Fleet membership is not fixed at dispatcher boot. A worker daemon **announces itself** over NATS and the dispatcher merges it into the live fleet with **no restart** — the fix for having to edit `DOCKER_NODES` + restart just to add a node or change its slot count. The daemon publishes a **fire-and-forget** `WorkerAnnounce { node, slots, version }` on the plain (non-JetStream) subject `event.worker.announce` every ~15s; the dispatcher subscribes and forwards each announce **into the single-writer actor** as a mailbox message (`Msg::WorkerAnnounce`), so every fleet mutation happens on the actor thread — no shared registry, no locks over the decision, exactly like every other state change. Semantics:
- **Slot source.** A worker advertises **its own** capacity (`WORKER_SLOTS`, default 4) in the announce; re-announcing with a new value changes capacity at runtime (e.g. air 4→5), no restart.
- **Precedence and merge.** `DOCKER_NODES` remains the supported **static seed**. An announce merges by node name and the **live announcement wins**: a matching worker seed (or a previously-announced node) has its slot count, version, schedulability, and reachability refreshed in place; an unknown name **joins** as a new worker node with its own `req.worker.{node}.>` RPC channel; a name already held by a **docker-endpoint** seed is refused (an announce cannot repurpose a directly-driven daemon into a NATS-proxied one). New capacity is seen **immediately** — the actor re-drains the §3.5 launch capacity queue on the same turn, so launches parked for no-capacity fire onto the fresh node.
- **Heartbeat loss = deregistration.** The announce doubles as a heartbeat. A dynamically-announced node whose heartbeat lapses past a timeout (default 60s, comfortably above the announce interval) is marked **unschedulable**: placement skips it and the platform snapshot shows it down, but its **already-running containers are never killed** — `route`/`wait`/`inspect` still reach the node, and the poll-based `wait` re-attaches on reconnect (§3.6 semantics unchanged). Queued launches targeted at a lost node simply wait for other capacity. A fresh announce re-admits it. **Static (`DOCKER_NODES`) seeds are never heartbeat-gated** — they keep using the ping-based out-of-service path above, so a seed that also re-announces (to change slots) is not deregistered merely for going quiet.
- **Zero-seed boot (interacts with the §3.6 fleet-level startup rule).** The dispatcher may boot with **zero configured nodes** (`DOCKER_NODES` present but empty): startup **succeeds** with zero capacity, and launches queue via the NoCapacity path until the first worker announces and supplies capacity. Only a *configured* fleet with no reachable node with slots > 0 is still a fatal misconfiguration (the crash-loop guard).
- **UI surface.** Announced nodes flow into the live-fleet records (`fleet.status` and the config snapshot's `nodes`, §3.1 above), so the operator UI reflects live fleet membership — joins, slot changes, and heartbeat-loss deregistrations — not just the boot `DOCKER_NODES` list.
- **NATS permission.** A worker daemon's minted creds (`chuggernaut admin worker-creds`, §7.4) must **allow publish to `event.worker.announce`** — in an operator-mode server a non-empty publish allow-list is strict, so without this grant the announce is denied and dynamic registration silently no-ops. The grant rides in `auth::nats::worker_permissions` alongside `_INBOX.>`; **existing worker creds must be re-minted on deploy** for it to take effect. Only a backend that can actually route to announced nodes (the worker fleet backend) acts on announces — a single-node Docker deployment drops a stray announce rather than inserting a node it can never place work on.
...
- **Opt-in per node.** The worker reads `WORKER_CACHE_DIR` (a host path on the node). Unset ⇒ caching is off and nothing changes. Set ⇒ the daemon creates/owns the directory and enables the cache.
- **The bind-mount.** When enabled, the worker's own Docker backend adds a single host bind-mount of the cache dir into every container it launches, at a fixed container path (`/cache/sccache`). The dispatcher's Docker backend never sets this, so a plain fleet stays bind-mount-free (§3.1 "no host bind-mounts"). The cache carries **no job state** — it is a build accelerator only, so a missing, empty, or cold cache is always safe.
- **The env.** The daemon injects `RUSTC_WRAPPER=sccache` and `SCCACHE_DIR=/cache/sccache` into the launched container's environment — added purely from the node's own config, with zero dispatcher or wire involvement. `sccache` is baked into the agent-rust image; if it is ever absent, `RUSTC_WRAPPER` pointing at a missing binary degrades to an ordinary uncached build rather than failing.
- **Concurrency & ownership.** The host cache dir is created and owned by the worker daemon at startup, not by any container. Concurrent containers on the same node share the one cache dir; sccache is built for concurrent access (it locks), so sharing is safe.
...
**Warm-target seeding (image property, not dispatcher machinery).** A build image MAY bake a prebuilt `target/` so containers start from "latest main already built" rather than compiling cold every task. This is entirely an **image concern** — like the node-local build cache (§3.1) it adds nothing to the launch message, the wire, or the dispatcher, and touches no per-job state. The `agent-rust` image (`deploy/prod/Dockerfile.agent-rust`) compiles the whole workspace at image-build time into `CARGO_TARGET_DIR=/opt/chug-prebuilt-target` (a path *outside* `/workspace`, baked as an image ENV), then deletes the build tree; the runtime clone into the empty `/workspace` reuses those artifacts from the image's read-only layer, and each container's writes land on its own copy-on-write overlay, so the seed is shared safely and copied never. Registry-dependency artifacts (checksum-keyed) reuse regardless of the clone's mtimes — the reliable win; workspace crates are mtime-sensitive and recompile against a fresh clone, a residual left to sccache (§3.1) and the warm dep graph. The seed is as fresh as the last image rebuild — the worker self-refresh on every deploy (§3.1) keeps it warm, so no extra staleness machinery is needed. It composes with the node-local cache: the seed warms the image's read-only layer, sccache reuses compiler invocations across jobs on the node.
...
**Sharing implication:** because the wizard reuses the `global/agents` agent credential by default, **wizard usage bills the same Claude subscription/account as the agent containers** — and, more sharply, **contends for the same rate limits.** When agents are mid-run the account can be rate-limited, so the wizard's request may be throttled (429) or the platform overloaded (529). The wizard therefore retries transient capacity errors with exponential backoff (honoring the `retry-after` header, total sleep capped at ~20s so the synchronous chat turn never blocks for minutes) and, if still limited, returns a friendly *"the platform model is busy — try again in a moment"* message rather than the bare status code; a 401 instead surfaces as *"wizard credential invalid; check `CLAUDE_CODE_OAUTH_TOKEN`"*. A cheaper `WIZARD_MODEL` reduces cost but not contention (same account, same limits); set `WIZARD_API_KEY` to a separate credential to isolate the wizard from agent load entirely.

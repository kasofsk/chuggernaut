...
| `web` (not a crate) | 🟡 core done | `web`: Vite + React 19 + TS, no framework state — SSE-triggered refetch. Login, project chooser, job table + create/release/revoke, operator inbox (Pass/Fail + escalation Retry/Resolve/Revoke keyed on job state), job detail (tasks, per-file collapsed diff with colored hunks, live event log, **per-task transcript/log viewer + channel timeline**). PWA manifest/service worker + React Flow graph view still pending |
...
| `webhooks` | 🔴 stub | |
| `test-utils` | ✅ done | `FakeBackend`, `FakeProvider` (async run hooks mirroring submit-ack-then-exit ordering), `NatsTestServer` (shared: `CHUG_TEST_NATS_URL` or Docker `nats:2-alpine`; private: a local `nats-server -js` process per caller, else a private container — skip-guarded; `spawn_with_config` boots operator-mode servers), `TempRepo`/`WorkClone`/`clone_branch_from` |
...
- **Live dev stack (`deploy/dev/`, session 6)**: `compose.yaml` (operator-

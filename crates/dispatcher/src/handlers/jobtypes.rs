...
//! project-owned, so both verbs read `.chug/jobs/*.yaml` at default-branch HEAD
...
//!   `.chug/jobs/_defaults.yaml` — the platform's view of what actually runs).
...
/// One job type in full for the library UI: raw YAML as authored and the path
/// it resolved to, plus the parsed type with `.chug/jobs/_defaults.yaml` merged
/// (the platform's view of what actually runs). Parse/field-rule failures come
...
async fn get_job_type(repos: &RepoManager, owner: &str, project: &str, name: &str) -> Vec<u8> {
    ...
    let file = match crate::project_config::read_file(
        repos,
        owner,
        project,
        &head,
        &format!("jobs/{name}.yaml"),
    )
    .await
    {
        Ok(Some(f)) => f,
...
/// Enumerate top-level `.chug/jobs/{type}.yaml` at default-branch HEAD, minus
/// the `_defaults.yaml` overlay and other `_`-prefixed helpers, with the
/// display metadata the create-form type picker shows. A file that fails to
/// parse still lists (stem only) — a broken type stays visible, not invisible.
async fn list_job_types(
    ...
    for stem in stems {
        let parsed = crate::project_config::read_file(
            repos,
            owner,
            project,
            &head,
            &format!("jobs/{stem}.yaml"),
        )
        .await?
        .and_then(|file| types::JobType::parse(&file.content).ok());

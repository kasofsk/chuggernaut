...
- **Unknown or ambiguous is a hard refusal naming what IS installed.** A version the node does not install **fails the launch** (`Launch`, never `NoCapacity`) listing the `xcode:` references it can serve; the match is exact, so `xcode:26` selects nothing. Two bundles reporting one version can differ in build, so that version is advertised nowhere and refused here naming both bundles — picking one would be exactly the silent wrong-toolchain build this scheme exists to prevent. An `xcode:` reference reaching a node that serves no host mode is refused as the placement bug it is.

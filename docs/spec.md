...
| `mode: host` | Top-level `image` **disallowed** — a host task has no image — and `runtime.env` **required**, because a host task with no declared environment runs against whatever the node's bare `PATH` holds. A **level** carrying its own `image` (an evaluator, `wrap_up`) is a container task regardless and does not inherit `runtime`, which is what makes the `ci` evaluator `.chug/jobs/_defaults.yaml` appends to every job type stay a container task: host work, container CI, one job. The evaluator and `wrap_up` image requirements are narrowed to match — under host, a level with no `image` of its own needs none. **The declaration selects the node**: placement excludes every node that does not advertise the mode (§3.1 placement, design #309 P2), so a host job type is placed on a host-capable node without a pin — and queues, loudly, on a fleet where none advertises it. **`work.type` must be `command`**: `agent` and `human` work under `mode: host` is an `Invalid` field rule on `work.type`, and the node refuses an agent-shaped host launch as well, covering a job type that reaches it by a `placement.node` pin. The original reason — a transcript harvested from a path derived from the cwd a host task moves (design #322 §2) — no longer holds: since design [#490](design/490-agent-work-on-a-mac.md) slice 1 the transcript is *resolved* by session id (§3.1) rather than computed. The refusal stands on what is still unbuilt — a launch-time capability test in place of this one (#490 D5), which slice 5 lifts together with the field rule. Neither of the capabilities it was waiting on is missing now: a node whose modes name `host` is installed with its own host channel binary since #490 slice 3 (§14 below), and since slice 4 it **discovers** an agent CLI on the daemon's own `PATH`, advertises whether it has one (`agent_cli`, §3.1) and refuses an agent-shaped host launch **by name** when it has none |
...
- **An agent-shaped host launch on a node with no CLI is refused by name** (`Launch`, never `NoCapacity`), naming the `PATH` that was searched. Until #490 slice 5 an agent-shaped host launch is refused on that shape alone in any case (§1.1's `work.type` rule and the node's own guard); what the probe changes is that a node now knows whether it *could* serve one and says which capability is missing when it cannot.

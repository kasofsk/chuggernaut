...
**Two qualifiers travel with the Linux result.** Every Linux run of those assertions so far needed `XDG_RUNTIME_DIR` present in the environment that invokes the daemon, and whether a daemon started by a supervisor is given one is **unsettled** (#440 slice 7). And the `--expand-environment=no` the scoped launch path passes matters only on **systemd v258 and later**, the release where that client began rewriting a task's own command line before exec'ing it; the proving node ran 260. Below v254 the option does not exist at all and such a node refuses to advertise `host` rather than launching under a supervision it cannot get.

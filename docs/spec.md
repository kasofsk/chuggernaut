...
**What that guarantee covers, and what it does not.** It is unconditional for a task that is a **container** and conditional for a task that is a **host process** (`runtime.mode: host`, §1.1): a container task is the daemon's *sibling* on the node's docker, so no restart of the daemon can reach it, while a host task is the daemon's *child* and survives only while a mechanism keeps it out of the daemon's teardown set. The host half is stated because the mode is declarable and now placeable, not because anything runs there today — `validate()` accepts `runtime.mode: host` (§1.1) and placement routes by it (§3.1 placement, design #309 P2), and `.chug/jobs/mac-proof.yaml` (design [#490](design/490-agent-work-on-a-mac.md) slice 6) is the one job type in this repo declaring it, but no host task has yet been released, so the host half stays unexercised rather than unreachable. Case by case (design [#440](design/440-native-worker-daemon.md) §3, [#309](design/309-host-native-execution.md) §6):

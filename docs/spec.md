...
**Docker reachability on a node** (design [#517](design/517-docker-access-for-jobs.md) D4). Every daemon probes once at boot whether it can reach a docker endpoint and advertises the answer as `docker_reachable` (capability advertisement, above), for **both** execution modes: a container node whose launches a node-side grant could bind the socket into, and a host node whose tasks reach it ambiently through the uid they run as.

...
pub struct Job {
    ...
    pub eval: Vec<Evaluator>,              // additive per-job evaluators, layered on top of the type's eval list at execution; the type's evaluators are a floor — creation can add criteria, never remove or override them; name collisions are a release-time error (see docs/reference/design-lifecycle.md)
...
**Migrating a project** moves the directories and rewrites the paths its job types name (`prompt:`, `run:`), which are ordinary repo paths and follow the files. Because the *deployed* dispatcher is what reads them, a project whose platform predates the config root must deploy a dispatcher that understands `.chug/` **before** the move lands; otherwise its job types become unfindable and no job can be created until the deploy catches up (§14, `docs/reference/runbooks/adhoc-deploy.md`).
...
runtime:                       # optional; where this type's tasks run and against which toolchain. Absent = mode: container with no declared environment, which is every job type that predates the block
  mode: container | host       # optional; default container. `host` is a legal declaration (design #309 P1): top-level image disallowed, env required. It selects the node (#309 P2): placement excludes every node not advertising the mode (§3.1 placement)
...
resources:                     # optional; disallowed for work.type: human
  cpu: number                  # declaring this or memory: constrains placement to nodes that enforce them for the launch's mode (§3.1 capability advertisement, design #309 §7); a host-mode level is bounded by no cgroup, so the two are mutually exclusive with runtime.mode: host
  ...
  task_timeout: duration       # per-container execution limit; default 1h. Enforced dispatcher-side by the §3.5 timeout scan rather than by a runtime, so it is mode-independent and constrains placement not at all
...
wrap_up:                       # optional; the job's third step (work → evaluation → wrap-up); see docs/reference/design-lifecycle.md
...
| `mode: host` | Top-level `image` **disallowed** — a host task has no image — and `runtime.env` **required**, because a host task with no declared environment runs against whatever the node's bare `PATH` holds. A **level** carrying its own `image` (an evaluator, `wrap_up`) is a container task regardless and does not inherit `runtime`, which is what makes the `ci` evaluator `.chug/jobs/_defaults.yaml` appends to every job type stay a container task: host work, container CI, one job. The evaluator and `wrap_up` image requirements are narrowed to match — under host, a level with no `image` of its own needs none. **The declaration selects the node**: placement excludes every node that does not advertise the mode (§3.1 placement, design #309 P2), so a host job type is placed on a host-capable node without a pin — and queues, loudly, on a fleet where none advertises it. **`work.type` is unrestricted** (design [#490](design/490-agent-work-on-a-mac.md) slice 5): `agent` work under `mode: host` validates, and what refuses it is the **node**, per launch, when it cannot serve one. The field rule that forbade it (#322 §2 N2) is superseded — it protected a transcript harvested from a path derived from the cwd a host task moves, and the transcript is resolved by session id (§3.1) instead. What guards the case is a capability test at the launch (#490 D5, §3.1 below): an agent-shaped host launch on a node with no discovered CLI, or no runnable channel binary of its own, is refused **by name** — never at boot, so a Mac missing one keeps every container and command-host slot it has, and never on the shape, so a job type reaching a host node by a `placement.node` pin is judged on what the node can do rather than on what it declared |
...
A **schedule** is time-triggered job creation: one file per schedule under `.chug/schedules/`, resolved from default-branch HEAD like every other config directory (design [#310](design/310-scheduled-jobs.md)). It is repo-versioned so a schedule change ships in the same commit as the job type it fires and clears the same gates. <!-- intent -->
...
**Abort verdict** (docs/reference/design-lifecycle.md): `abort: true` on an evaluator verdict (`submit_eval` for agent evaluators, `TaskResolution::Fail` for human evaluators) declares the work *not satisfiable by rework* — wrong premise, impossible requirement. It implies fail (a contradictory `pass: true, abort: true` submission normalizes to abort). At the eval reduce, an abort from any **required** evaluator skips the remaining rework budget and escalates immediately, carrying the aborting evaluators' structured findings in the escalation prompt. Advisory (`required: false`) aborts are recorded as plain advisory fails. Command evaluators have no abort — an exit code cannot judge fixability. `abort` on a human *work* task resolution is ignored (a declined work task escalates regardless).
...
|---|---|---|
| `artifacts` | 90d | Per-task blobs (transcripts, container logs, the transcript-missing marker of §3.2) **and** per-job attachments (§1.6); chunked internally, so blobs are not bound by `max_payload`; gzip + age-encrypted at rest under the `age_artifacts` key |
...
The dispatcher re-validates static configuration (job type file, all prompt paths for agent and human evaluators, and the job's supplied inputs against the type's `inputs:` declaration) at `base_ref`. If re-validation fails, the job transitions to Stalled with a Human task describing the missing file. The inputs re-check is there because the declaration is a file-derived fact pinned to `base_ref` — a type that grew a `required` input between release and unblock must fail here. Secrets and vars are not re-checked at this point — their values are live KV entries, not pinned to `base_ref`. The same write that pins `base_ref` materializes declared input defaults, exactly once (§1.1).

**Launch-time validation:**

Secrets and vars declared in the job type are checked immediately before injection. If any are missing, the job transitions to Stalled.
...
- **Placement**: at launch, pick the in-service node under the platform's **placement policy** (`PLACEMENT_POLICY`, §12.4 — one setting for the whole fleet, not per job type). Two policies:
  ...
  Both policies read the same ping-provided inputs (slots + running count); no policy needs a new RPC, and both exclude full/0-slot and out-of-service nodes identically. No preemption. **A node that does not serve the launch's execution mode is excluded before any of that** (design [#309](design/309-host-native-execution.md) §5a): the mode a launch requires is read off its `image` — present is `container`, absent is `host`, the same selector the node's own backends route on ("Mode routing on a worker node" above) — and a candidate is eligible only if the modes it advertises (capability advertisement, below) contain it. The filter changes nothing for a container-only fleet, where every node advertises `container` and every launch requires it. When no eligible node has a free slot, placement returns a distinct **no-capacity** signal (`BackendError::NoCapacity`) — a *transient* condition, not a launch failure: the dispatcher queues the launch and retries when a slot frees (see §3.5 launch capacity queue), rather than failing the task. The one affinity control is an **optional pin**: a job type may set `placement: { node: <name> }` (§1.1), which threads through `ContainerLaunchConfig.node` and forces every container that job launches onto the named node — this **overrides the policy entirely**. A pin is honored or it fails — a pinned node that is full or out-of-service yields the same transient no-capacity signal as the unpinned case (queued and retried, no spillover onto another node); an unknown pinned name is a hard launch failure naming the known nodes. **A pin routes; it does not exempt.** A pin onto an in-service node that does not serve the required mode is a hard launch failure too, naming the node, the modes it serves and the one required: the node's own backend would refuse the launch by name anyway, and only a config change — to the node's `WORKER_MODES` or to the job type — can clear it, so queueing it would be a silent wait with a known answer — and that holds whether or not the node has a free slot, so a pin onto a live incapable node that is also full is the hard failure rather than the no-capacity signal. That verdict needs the node's live answer, so it is reached only *after* the **out-of-service** check (and only that one): a node that has never answered a probe advertises nothing and is transient, not misconfigured.

  The two no-capacity conditions are **distinguished by their message**, because they are different operator problems: eligible nodes exist but none has a free slot is `no free slots on any node` (a busy fleet, which clears on its own), while no node in the fleet advertises the required mode at all is a configuration error and says so, naming the mode and what each node does serve. Both stay transient by the §3.5 contract — a node declaring the mode joins without a dispatcher restart — but the second also raises a **fleet-wide warning**, bounded to one line per mode per fifteen minutes, so a job type requiring a mode nothing serves is loud rather than a queue that never drains. A fleet with *no* nodes at all is the first message, not the second: zero nodes is a fleet that has not registered yet (zero-seed boot, below), not one that refuses the mode. The node name cannot be checked offline (the fleet list lives in the dispatcher's env), so `validate`/release only shape-check it (`[A-Za-z0-9_-]+`). No labels, no anti-affinity, and **no per-run mechanism**: that pin is a job-type field, and nothing on the job record — inputs included — participates in `ContainerLaunchConfig.node`. Worker-node free slots come from the ping reply (the worker counts its own running `chuggernaut.managed` containers). The active policy is surfaced in the platform config snapshot (§6) so the UI can show how the fleet schedules.
...
- Container-internal files (MCP binaries, prompt, events batch) are **injected via the put-archive API** into the created container before start (see §4.2, §4.3) — no host bind-mounts, so nothing needs to exist on remote node filesystems (worker nodes substitute their node-local static artifacts, below). The permitted exceptions are a small closed class of **worker-provisioned node properties**, each added worker-side and never a launch input: a **node-local build cache** (see "Node-local build caching" below), a single writable mount that carries **no job state** — a build accelerator only, safe to be empty/cold, and never affecting correctness once mounted; the **read-only toolchain mounts** of a KVM launch (see "Node-local device passthrough" below), which are the opposite — required, and fatal to the launch when absent; and the **docker socket** of an allow-listed launch (see "Node-side docker socket grant" below). The third member is justified differently from the other two, and the difference is stated rather than smoothed over: the cache carries no job state and the toolchain mounts are read-only, while the socket is a **capability** — a granted launch holds node root. It is in the class by a decision (design [#517](design/517-docker-access-for-jobs.md) D1/D3), not by a property, which is why it is the only member the node grants per `(project, job type)` instead of per node.
...
**Worker nodes (`chuggernaut worker`).** A worker node runs a daemon that connects OUT to NATS and executes container operations against its local Docker socket — no Docker endpoint is exposed on any network and the node has no listening port. The dispatcher's fleet backend proxies each `ContainerBackend` op as JSON request-reply on `req.worker.{node}.{op}` (`launch`, `kill`, `inspect`, `copy_file`, `find_file`, `logs`, `ping`); `wait` is implemented dispatcher-side as an inspect poll, so worker restarts are transparent (containers keep running; the poll re-attaches). The daemon authenticates with scoped credentials minted by `chuggernaut admin worker-creds --node {name}` (subscribe `req.worker.{name}.>` + inbox reply publish — nothing else).

**Small-message discipline**: every worker request fits NATS's default 1MB max_payload. The launch request carries metadata plus small dynamic files inline (prompt, per-job credentials, harness config — KBs); **static artifacts are node-local**, provisioned at deploy time with the worker itself (the agent images, and the channel MCP binary baked into the worker's image at the same git SHA). A launch references them by name (`"channel"`) and the daemon substitutes its local copy; an unknown name fails the launch. The client side enforces a payload-size guard — a launch that exceeds it is a wiring bug (bulk bytes leaking back into the launch path), not a transport problem. **Replies are bounded too.** `logs` replies are tailed to the most recent ~700KB with a truncation marker; `copy_file` replies are bounded by the same `max_payload` but **refuse rather than truncate** — a partial file is worthless where a partial log tail is not — so a file whose base64 reply would not fit comes back as a named error carrying the path, the size and the bound, never as a reply that cannot be published and leaves the caller waiting out its op timeout. That bound is a property of the worker transport, not of `copy_file` itself: a container on a Docker-endpoint node is read in-process, with no NATS hop and no bound, so the same file that is refused on a worker node is copied whole there. A file legitimately larger than one reply — an output archive (§3.2) — travels over the separate **`copy_file_chunk`** op, which returns one bounded slice from a byte offset plus the whole-file length, so the caller reassembles it in a loop bounded by its own ceiling. The whole file is measured before the first slice is sent: one over the caller's ceiling comes back as the same named refusal, so an over-band read costs one round trip rather than a truncated archive. `copy_file_chunk` is **additive** — an N-1 daemon answers it with the unknown-op error, which decodes on both sides — so it does not bump `WORKER_RPC_VERSION` (§14.1), and during a mixed-version deploy window an output harvest against an un-refreshed node degrades to a logged miss. The ping reply carries the worker's build version and artifact hashes — and its current capacity and capabilities (both below) — and the dispatcher warns (never refuses) on version drift.

**Mode routing on a worker node** (design [#309](design/309-host-native-execution.md) §1). A node declares the runtimes it offers in `WORKER_MODES` (`container`, the default and what the whole fleet runs, and/or `host`) and constructs **exactly those** backends: a container-only node never touches the host backend, and a host-only node needs no Docker daemon at all. The launch request's **`image` is optional, and its presence is the mode selector** — a launch carrying one is a container task, a launch carrying none is a host process (`runtime.mode: host`, §1.1, which is why that mode disallows a top-level `image`). A node declaring both routes **per launch**, so the same node runs a host job type's work and that job type's container `ci` evaluator; every op after the launch routes by the container id the launch minted. A backend handed a launch in the mode it does not serve **refuses it by name** — a wrong-mode launch is a placement bug, never a silent fallback to the other runtime. Making `image` optional is a breaking change to an existing op's shape, so it moved `WORKER_RPC_VERSION` to **2** (§14.1); an N-1 daemon rejects an image-less launch as an unparseable payload, which placement by mode makes unreachable — a node that has not advertised `host` is never chosen for a host launch (Placement, above). Node-wide capacity is unchanged by the routing: any node offering `host` still refuses to boot above one slot. That rule outlived the `/workspace` collision that motivated it — design [#322](design/322-macos-native-runtime.md) §2's rebase gives each host task its own mapped workspace under `WORKER_HOST_ROOT` (§4.1) — and stays for phase 1 because a Mac's simulator state is machine-global.

**Host-node tenancy.** A worker node that serves `host` runs host work for the projects it names and no other (design [#309](design/309-host-native-execution.md) §10). Host mode gives a task the node itself — the task user's home, every cache under it, the node's process table — and the cache reuse that buys **is** the contamination risk, so single-tenancy is the honest form of accepting it. Like every other node property it is provisioned entirely worker-side: no wire field, no launch-config field, and **a job type cannot ask for it**. Mechanics:

- **Declared per node, fail closed.** `WORKER_HOST_PROJECTS` is a comma-separated `owner/project` allow-list matched against the launch's `JOB_PROJECT` — the same unshadowable stamp `WORKER_KVM_PROJECTS` reads (§4.1) — and **unset or empty runs host work for nobody**, so serving `host` at all and declaring whose work it is are two separate acts. A malformed or repeated entry is a hard config error rather than a tenancy that silently never matches.
- **A refusal, never a queue.** A host launch whose project the list does not name **fails the launch** (`Launch`, never the retryable `NoCapacity`), naming the project and the node: it can never clear without a config change on the node, so queueing it would be a scan tick's silence with a known answer.
- **Host launches only.** The list is read where a host launch is admitted and nowhere else, so a mixed-mode node keeps serving **container** launches for every project exactly as it did. A node that serves no `host` mode never constructs the backend that holds it.
- **Refused at the node, and at the deploy.** An empty list is a working configuration to the daemon — it warns at boot and refuses each host launch, rather than refusing to start and taking a mixed-mode node's container work down with it. `deploy/prod/build-worker.sh` therefore refuses a deploy that declares `host` in `WORKER_MODES` with no tenancy beside it, which is where an operator is standing when the omission is still cheap.

**Dynamic worker registration.** Fleet membership is not fixed at dispatcher boot. A worker daemon **announces itself** over NATS and the dispatcher merges it into the live fleet with **no restart** — the fix for having to edit `DOCKER_NODES` + restart just to add a node or change its slot count. The daemon publishes a **fire-and-forget** `WorkerAnnounce { node, slots, slots_max, capacity_epoch, capacity_generation, version, capabilities }` on the plain (non-JetStream) subject `event.worker.announce` every ~15s; the dispatcher subscribes and forwards each announce **into the single-writer actor** as a mailbox message (`Msg::WorkerAnnounce`), so every fleet mutation happens on the actor thread — no shared registry, no locks over the decision, exactly like every other state change. Semantics:
...
- **Capability advertisement — what a node can do, as opposed to how much** (design [#309](design/309-host-native-execution.md) §4). Both transports also carry an optional `capabilities` record: the execution `modes` the node serves (`container`, `host` — the `WORKER_MODES` it constructs backends for, §3.1 mode routing above), its `platform` (`{os}/{arch}`, diagnostic only), whether it enforces a task's `resources.cpu`/`memory` (`resources_enforced`), the named exclusive `leases` it holds, the node-interpreted `runtime.env` references it discovered it can serve (`envs` — the `xcode:<version>` set, below; a `nix:` reference names a build rather than a node fact and is never listed), and whether it discovered an **agent CLI** on the daemon's own `PATH` (`agent_cli`, below — design [#490](design/490-agent-work-on-a-mac.md) D3), and whether its daemon reached a **docker endpoint** when it probed at boot (`docker_reachable`, below — design [#517](design/517-docker-access-for-jobs.md) D4). Each field is **additive on both records and does not move `WORKER_RPC_VERSION`** (§14.1): a daemon predating one advertises nothing, and *absent* is read as `modes: [container]`, `platform: "unknown"`, `resources_enforced: true`, `leases: []`, `envs: []`, `agent_cli: false`, `docker_reachable: false` — a reading that fails closed (only a new daemon can claim host capability) and leaves the container fleet's behavior unchanged. **The `ping` reply is authoritative and the announce fills in only for a node no ping has answered for**, because an unclaimed capability is indistinguishable from a node that legitimately lacks it, whereas a ping is pulled at every placement attempt and so self-corrects; the dispatcher applies it inside its probe, on the reply path. There is deliberately **no `(capacity_epoch, capacity_generation)` ordering key** here — capabilities are boot-time facts that change only by recreating the daemon, so last-writer-wins on the pull path suffices. A **docker-endpoint** node has no wire path at all and would read as absent forever, so the dispatcher **synthesizes** the same absent reading for it from the node kind rather than from silence. Placement reads three fields of this record: the `modes`; — for a launch whose job type declares `resources.cpu`/`memory` — `resources_enforced`, which it reads **per the launch's resolved mode** (design #309 §7); and — for a launch whose level resolves a **node-interpreted** `runtime.env` — `envs`, which must contain that reference exactly (design [#543](design/543-placement-granularity.md) D2). `resources_enforced` is derived from the node's container runtime, so on a node serving both it answers for the container launches the Docker `HostConfig` bounds and never for the host ones, which are bounded by nothing; a launch declaring either resource field is placed only on a node that can bound it, queued as ordinary `NoCapacity` when none can, and refused outright (`Launch`, naming the field and the node) when a `placement.node` pin carries it to one that cannot. The environment takes the same refusal pair: queued as ordinary `NoCapacity` naming the reference and the fleet's advertisements when no node serving the mode advertises it, and refused outright (`Launch`, naming the reference and the node's set) when a `placement.node` pin carries it to one that does not — so the node's own resolution refusal (§3.1 `xcode:` mechanics, below) becomes the backstop for a launch arriving on a stale or absent advertisement rather than the primary check. A `nix:` reference is a build the node realises at launch and no node advertises it, so it is no placement input. `platform` is diagnostic, `agent_cli` and `docker_reachable` are advertised for the operator and for a later capability-filtered placement rather than filtered on today (design #490 D3, #517 D4), and `leases` is #309 P4.
...
3. **Replaces the daemon: installs the binary, asks the supervisor to restart.** The daemon is a **binary under a supervisor** — a systemd unit or a launchd agent over an environment file (design [#440](design/440-native-worker-daemon.md) D2/D6) — so the swap extracts `chuggernaut`, the channel binary and this script *out of the worker image the build phase just made* (`docker create` + `docker cp`, which keeps the daemon's build environment byte-identical and needs no toolchain on the node), installs each by renaming it over the live path, and asks the supervisor to restart the unit. **That extraction is a Linux node's, and only for the binaries the node itself runs**: the worker image is a Linux container, so a **macOS** node instead *compiles* the daemon in its build phase — from the same fetched context, with the node's own declared `cargo` (`WORKER_CARGO`, `WORKER_BUILD_DIR` in its run spec) — and the swap installs what was compiled, refusing when the staged build is not for the SHA the image carries. **The channel binary is the opposite case and comes out of the image on both platforms**: the node never execs it, it is injected into agent containers (§3.1), which are Linux, so a macOS node's own build of it is a Mach-O nothing in a container can start. **A node whose `WORKER_MODES` names `host` is installed with a second channel binary beside it** — since design [#490](design/490-agent-work-on-a-mac.md) slice 3, at its own path so the two coexist on a dual-mode mac — and that one inverts the case again: the node execs it *itself*, so it comes out of the node's own build on macOS and out of the image on Linux. Each staged binary is checked against **its own executor** before anything is installed, because a binary built for a foreign platform installs perfectly and then fails where nobody is watching: the daemon must exec on the node (a `--version` exec) or the supervisor loops it, the injected channel binary must be one an agent container can exec — on a macOS node that is its object format against the architecture the node's own container engine reports, since asking it to run on the node would be asking the wrong platform — and the host copy, whose executor *is* the node, is checked by being run there, with a Linux ELF found in that slot on a mac refused by name. Nothing is carried forward: the run spec is a **file on the node** the supervisor loads on every start, so a value survives because it is written down rather than because the swap copied it into a new `docker run`. **In-flight job containers survive** — nothing removes a container at all, and the dispatcher's poll-based `wait` (an inspect poll, above) re-attaches over the new daemon and still delivers the exit code. The record of a replacement that fails to start is the supervisor's own log (`journalctl -u chug-worker`, or the agent's log path on macOS); it cannot ride back to the dispatcher, because the daemon that would report it is the thing being replaced. Every refusal is evaluated **before** anything is installed, with the live daemon still serving: a node still running the *containerized* daemon (one no operator has converted with `build-worker.sh`) refuses its own swap and is deployed over ssh instead, and so does a node whose supervisor is unreachable or whose unit this daemon does not belong to — restarting a unit that does not own this process would leave two daemons on one machine.
...
**What that guarantee covers, and what it does not.** It is unconditional for a task that is a **container** and conditional for a task that is a **host process** (`runtime.mode: host`, §1.1): a container task is the daemon's *sibling* on the node's docker, so no restart of the daemon can reach it, while a host task is the daemon's *child* and survives only while a mechanism keeps it out of the daemon's teardown set. The host half is stated because the mode is declarable, placeable and exercised — `validate()` accepts `runtime.mode: host` (§1.1), placement routes by it (§3.1 placement, design #309 P2), and `.chug/jobs/mac-proof.yaml` (design [#490](design/490-agent-work-on-a-mac.md) slice 6) is the one job type in this repo declaring it, released and run on `gumbo-air-0`. What no host task has yet met is the event this paragraph is about: nothing has restarted or replaced a daemon underneath a live host task on the air, so each guarantee below still rests on the proof its own bullet names. Case by case (design [#440](design/440-native-worker-daemon.md) §3, [#309](design/309-host-native-execution.md) §6):
...
- **Host tasks, across the platform's own self-refresh — the refresh is refused rather than the task risked.** While any host task is live the daemon declines `refresh`, naming the task, both when the op is accepted and again at the swap boundary after the quiesce above, failing the refresh at its `drain` stage (#440 D4). The deploy then fails that node's leg by the confirmation rule above: a refresh that would have killed running host work reports why instead of performing it.
- **A reboot, or any restart broader than the unit — not guaranteed.** Nothing survives a reboot on either platform, and a host rebuild that restarts more than the daemon's unit is the same case. These are operator acts, and the operator's step ahead of them is to drain the node first (set its desired slots to zero, above; `docs/reference/runbooks/worker-capacity.md`). The self-refresh refusal exists precisely because holding a task outside the daemon's unit does not cover this case.

**Two qualifiers travel with the Linux result.** Every Linux run of those assertions so far needed `XDG_RUNTIME_DIR` present in the environment that invokes the daemon, because every one of them ran **unprivileged** and an unprivileged daemon can only create a `systemd --user` scope, which has to locate a bus. The supervised daemon does not: the unit #440 slice 7 declares runs as `root`, whose scopes are **system** scopes on a bus at a fixed socket path, so it borrows nothing from its environment. That is read off the launch path, not measured — no node runs the unit — and it holds only while the daemon is root. And the `--expand-environment=no` the scoped launch path passes matters only on **systemd v258 and later**, the release where that client began rewriting a task's own command line before exec'ing it; the proving node ran 260. Below v254 the option does not exist at all and such a node refuses to advertise `host` rather than launching under a supervision it cannot get.
...
- **Opt-in per node.** The worker reads `WORKER_CACHE_DIR` (a host path on the node). Unset ⇒ caching is off and nothing changes. Set ⇒ the daemon enables the cache and creates the directory **in its own filesystem view**, which is the host's only when the daemon runs natively on the node — a daemon still running containerized has that path unmounted, so its create lands in the container's writable layer. Either way **the host path is a node-provisioning prerequisite** (design #372 C3), for a different reason in each view: a containerized daemon's create never reaches the host, so the engine refuses every launch whose bind source is missing; a natively supervised daemon does create it at boot — before it serves anything — but only where it has permission to, so a path it cannot create is a config refusal at start that the supervisor then loops. The deploy provisions it (`sudo -n` as the fallback) before the daemon starts, so neither happens.
...
- **Opt-in per node, granted per project.** `WORKER_KVM` names the device (`1` ⇒ `/dev/kvm`); unset ⇒ nothing changes. `WORKER_KVM_PROJECTS` is a comma-separated `owner/project` allow-list matched against the launch's `JOB_PROJECT` — **empty grants nobody**, so enabling the device on a node and granting it to a project are two separate acts. The daemon **refuses to start** when the device is declared and its node is absent: a node never advertises a capability it cannot serve. The key it matches on is dispatcher-composed and unshadowable — `JOB_` is a reserved prefix (§4.1), so no `vars:` or `secrets:` declaration can move the value the allow-list is read against.
...
**Node-side docker socket grant.** A worker node MAY bind its own docker socket into the containers of allow-listed launches (design [#517](design/517-docker-access-for-jobs.md) D3). Like the device passthrough it is a **node property provisioned entirely by the worker daemon**: no wire field, no launch-config field, no job-type field, and the dispatcher's own Docker backend never sets one. **A job type cannot ask for it** — the requirement lives in the node's config, so granting node root takes an operator act outside the repo rather than a merge (design [#309](design/309-host-native-execution.md) §10's surviving mechanism clause, [#517](design/517-docker-access-for-jobs.md) D2). Mechanics:

- **Opt-in per node, granted per `(project, job type)`.** `WORKER_DOCKER_SOCKET` names the host socket (absolute path; unset ⇒ nothing is bound anywhere). `WORKER_DOCKER_GRANTS` is a comma-separated `owner/project:job_type` allow-list matched against the launch's `JOB_PROJECT` and `JOB_TYPE` — **empty grants nobody**, so binding a socket on a node and granting it to a workload are two separate acts. Both stamps are dispatcher-composed and unshadowable (`JOB_` is a reserved prefix, §4.1), and `JOB_TYPE` is the job type's own `name:`.
- **Work level only.** The match additionally requires `CHUG_PHASE=Work` (§4.1, §6.3), so an allow-listed pair holds the socket for its work and wrap-up commands and **no evaluator of it holds one** — including the `ci` evaluator project defaults append, which the job type's author never wrote (design [#543](design/543-placement-granularity.md) D5, amending [#517](design/517-docker-access-for-jobs.md) D3). The level is read from that `CHUG_`-prefixed stamp and never from the `CHANNEL_ROLE` beside it: only the first is sealed against a job type's `vars:` (§5.3), and matching the second would hand a job type a way to declare itself work level and re-obtain node root.
- **Fail closed at every layer, loudly.** A malformed or repeated allow-list entry, or a socket path that is not absolute, is a hard config error rather than a grant that silently never matches. The daemon **refuses to start** when the socket is declared and absent **from the daemon's own view** — a containerized daemon needs it mounted into `chug-worker` itself, the same node-provisioning precondition the nix paths carry below — because a node never advertises a capability it cannot serve, and a grant that quietly binds nothing is worse than no grant.
- **The bind.** An admitted launch gets the node's socket at `/var/run/docker.sock`, writable — a client cannot connect through a read-only bind — and gets nothing else: no device, no toolchain mount, and no injected `DOCKER_HOST`, because a docker client with no endpoint named looks at exactly that path. Declared as a typed bind mount, so a missing socket is refused by the engine at create rather than materialized as an empty file. Any other launch on the same node gets none of it.
- **What it grants, stated plainly.** Node root. A container holding the socket can bind-mount the host filesystem, read the daemon's own key mount, and from there the node's NATS credential — so it can receive other jobs' minted credentials. This is **accepted, not mitigated** (design [#517](design/517-docker-access-for-jobs.md) D1), and it holds only while every job on the allow-list runs code the operator wrote or vendored. A project whose origin takes third-party merges does not belong on the list.
...
- **Opt-in per node.** `WORKER_NIX_GCROOTS_DIR` names a worker-writable host directory; unset ⇒ no realise and no roots, and nothing changes. `WORKER_NIX_CLIENT` (default `/nix/var/nix/profiles/system/sw/bin/nix-store`), `WORKER_NIX_DAEMON_SOCKET` (default `/nix/var/nix/daemon-socket/socket`), `WORKER_NIX_STORE_DIR` (default `/nix/store`) and `WORKER_NIX_REALISE_TIMEOUT_SECS` (default 30) are the remaining knobs. The daemon **refuses to start** when the roots directory, the client or the socket is absent **from its own view**, or when the toolchain path it will realise does not **resolve into the store** in that same view. Those refusal conditions never change; **what the view IS** does, with how the daemon is supervised. A **natively supervised** daemon's view is the node's, so all of these are node state rather than mounts, and the toolchain path need only *resolve* into the store — any number of symlink hops, a NixOS `environment.etc` entry's `/etc/static` included. A daemon still running **containerized** needs the same host paths mounted into the **`chug-worker` container itself** (the store read-only, the profiles tree read-only, the socket directory read-write, the roots directory read-write **at the same path as on the host**, since the nix daemon resolves a root path in its own namespace, and — on a node that attaches a KVM device — the **directory holding** the declared toolchain path, read-only at the same path); the toolchain's *parent* rather than the path itself, because the client resolves `--realise`'s argument before the nix daemon hears anything and a bind whose source is the operator's symlink has it resolved away by `mount(2)`, leaving a non-store path the client refuses — so on that node alone the declared path must be a **direct** symlink into the store. Both are node-provisioning preconditions rather than assumptions. Nothing is added to a *launched* container by any of this — the closed class of launch-time node properties above is unchanged.
...
- **Granted per project, node-side, fail-closed.** `WORKER_NIX_PROJECTS` is a comma-separated `owner/project` allow-list matched against the launch's `JOB_PROJECT`, exactly as `WORKER_KVM_PROJECTS` is, and against the same unshadowable stamp (§4.1); unset or empty **grants nobody**. A job type asks for an *environment*; it never asks for a *privilege*, so no config a project can merge widens this. **Granting it grants evaluation**: flake evaluation is client-side and unsandboxed, so an allow-listed project's own flake code runs inside `chug-worker`, beside the docker socket, the NATS creds and the git key. That is tolerable only under the single-tenancy rule above.
- **Refused, never dropped.** A launch declaring a **`nix:`** `runtime.env` on a node that realises no environments, or for a project the node does not allow-list — and a launch of **any** scheme naming one the node serves in no mode (the registry is `nix:` and `xcode:`, below) — **fails the launch** (`Launch`, never `NoCapacity`) with a message naming the reference, the project and the setting. The two node-nix preconditions are this scheme's alone: the node forks on the scheme first, so an `xcode:` reference takes neither the GC-root directory nor the project allow-list, and `xcode:` states its own refusals below. Falling through to a container without the toolchain is the silent-drop class this design exists to close; the bootstrap guard (§4.1) closes the remaining N-1 case.
...
**Node-interpreted environments: `xcode:<version>`** (design [#322](design/322-macos-native-runtime.md) §3). `runtime.env` is **required** under `mode: host` (§1.1) and Xcode is not expressible as a flake output, so the scheme — not the field — is what grows: `runtime.env` keeps its exact contract (opaque, declared in the project repo, resolved by the node) and the node resolves `xcode:` from what is installed on the machine. Mechanics:

- **Discovered, never operator-typed.** A node whose `WORKER_MODES` names `host` scans `/Applications/Xcode*.app` **once at boot** and reads each bundle's `Contents/version.plist` (XML, where `Info.plist` is a binary plist) for its `CFBundleShortVersionString`. Static node config would relocate a physical fact into a file that goes silently wrong after a machine is rebuilt (design #309 §4); a bundle with no `Contents/Developer/usr/bin` — the directory the task's `PATH` is pointed at — or no readable version is not an install, so what the node advertises is what it can actually serve. A node serving no host mode scans nothing.
- **Selected per task through `DEVELOPER_DIR`, never `xcode-select -s`.** The resolved bundle's `Contents/Developer` is exported into the task's environment, and `CHUG_ENV_PATH` names `Developer/usr` so the §4.1 bootstrap guard puts `xcodebuild`/`xcrun` at the head of `PATH` under the same rule every declared environment gets. `xcode-select -s` mutates a machine-global symlink: two tasks on different Xcodes would fight over it and a crashed task would leave the machine changed. No GC root is taken — an app bundle is not a store path, so there is nothing to hold and nothing to release.
- **Unknown or ambiguous is a hard refusal naming what IS installed.** A version *reaching* a node that does not install it **fails the launch** (`Launch`, never `NoCapacity`) listing the `xcode:` references it can serve; the match is exact, so `xcode:26` selects nothing. Two bundles reporting one version can differ in build, so that version is advertised nowhere and refused here, naming both bundles, when a launch reaches the node anyway — picking one would be exactly the silent wrong-toolchain build this scheme exists to prevent. Since design [#543](design/543-placement-granularity.md) D2 placement matches the declaration against `NodeCapabilities.envs` first (§3.1 capability advertisement, above), an unadvertised version — the uninstalled and the ambiguous alike — is normally refused there instead: queued as `NoCapacity` unpinned, `Launch` under a `placement.node` pin. This refusal is what a launch arriving on a stale or absent advertisement still meets. An `xcode:` reference reaching a node that serves no host mode is refused as the placement bug it is.
- **Finding none is a warning, not a refused boot.** A host-capable node with no servable Xcode logs it and advertises an empty set; the refusal happens at the launch that asked. Refusing the boot would take a **dual-mode** node out of the fleet for every container job placed on it, and a host-capable Linux node legitimately has no Xcode at all.
- **An unpinnable environment is recorded by the task, not by the platform.** A version string is not a full pin — two Macs on "the same" Xcode can differ in build, in command-line tools and in installed simulator runtimes — so a host job type's task script **logs `sw_vers`, `xcodebuild -version` and `xcrun simctl runtime list`** into the task's stdout, which the platform already captures and encrypts (§3.2). The record has to come from the tools themselves, and only the task's own shell can produce it; the node logs the version, build and `DEVELOPER_DIR` it selected. A build that cannot be pinned is at least auditable after the fact.

**The agent CLI on a host node** (design [#490](design/490-agent-work-on-a-mac.md) D3). A container task's CLI comes out of its image; a host task has no image, so the CLI is resolved as a bare `claude` on the **daemon's own `PATH`** — on a Mac the one `deploy/prod/install-worker-launchd.sh` renders into the launchd agent.

- **Discovered at boot, never operator-typed.** A node whose `WORKER_MODES` names `host` scans that `PATH` once for an executable `claude` and advertises the answer as `agent_cli` (capability advertisement, above); a node serving no host mode scans nothing and advertises `false`, which is also what a daemon predating the field reads as. The probe reads the filesystem and does not run what it found — pinning a CLI version is not correctness-critical, the transcript being resolved rather than computed (#490 D1).
- **Finding none is a warning, not a refused boot**, for the reason the Xcode scan gives: a dual-mode Mac that cannot serve agent work still serves every container launch and every command host launch, so the refusal belongs at the launch that asked (#490 D5).
- **An agent-shaped host launch on a node with no CLI is refused by name** (`Launch`, never `NoCapacity`), naming the `PATH` that was searched. Since #490 slice 5 that refusal is the *whole* test — agent shape alone is admitted — and it has a second arm: a node holding no runnable channel binary of its own at `/usr/local/lib/chuggernaut/chuggernaut-channel-host` is refused naming **that**, because a host task's MCP server is the node's file rather than an injected one (#490 D2, §4.2 below). The daemon discovers both facts at boot and hands them to the host backend, which asks them of each launch that sets `CLAUDE_CONFIG_DIR`.
- **A host agent task's channel MCP server is named, never sent.** The dispatcher injects the channel binary into a **container** at `/usr/local/bin/chuggernaut-channel` and, for a host task, injects nothing and names the node's own copy — routed by the launch's `image`, the same selector every backend routes on (§3.1 mode routing). Nothing could inject it: the binary a Mac execs is a Mach-O, and `/usr/local/bin` is outside the wire-path mapping (§4.1) in any case.
- **`CLAUDE_CONFIG_DIR` is a rebased env value on a host node** (§4.1's fourth surface), so the CLI writes its transcript tree inside the task directory and the `find_file` the harvest runs against `/chuggernaut/claude/projects` resolves in the same tree. That tree is the one leaf the task's own credential teardown spares — the harvest reads it after the process has exited, so it lives until the task directory is removed, the same bound `logs` and `copy_file` have, while every injected credential beside it is still deleted the moment the command returns (#490 D6). The agent's **command** resolves its own `/chuggernaut` paths through `$CHUG_HOST_CREDS` for the same reason `bootstrap_cmd` resolves the clone destination through `$CHUG_WORKSPACE`: a launch's `cmd` is the one surface the rebase does not reach. A container's command line is unchanged to the byte.

**Docker reachability on a node** (design [#517](design/517-docker-access-for-jobs.md) D4). Every daemon probes once at boot whether it can reach a docker endpoint and advertises the answer as `docker_reachable` (capability advertisement, above), for **both** execution modes: a container node whose launches a node-side grant binds the socket into ("Node-side docker socket grant", above), and a host node whose tasks reach it ambiently through the uid they run as.

- **The claim is narrow, and deliberately so.** It says *this node's daemon reached a docker endpoint*; it does **not** say any given launch is handed the socket. Container launches receive nothing they are not granted node-side, so the flag is an audit record — it is what makes a node's docker access visible at all, and no design may read an absence off a node that never advertised one.
- **Resolved the way the docker CLI resolves it**, because on a host node the CLI in the task and the daemon share a `HOME` (§4.1's inherited floor): `DOCKER_HOST`, then the active context under that `HOME`, then the node's configured `WORKER_DOCKER_ENDPOINT`, first one that answers. The probe is a read — the API's own `GET /_ping` — so discovery creates, starts and stops nothing, and each candidate is bounded by a timeout.
- **Reaching none is not a refused boot.** A node with no docker is an ordinary node and differs by exactly the advertised `false`; a daemon predating the field advertises `false` too, having run no probe. Withholding host-mode access is not possible here at all — that needs per-task users (design [#309](design/309-host-native-execution.md) P3), which is why D4 advertises rather than enforces.
...
pub trait ContainerBackend: Send + Sync {
    ...
    /// Wire paths of every file under `dir` named `name` (design #490 D1a) — a
    /// list, so "several" is distinguishable from "one", bounded so a
    /// pathological directory refuses rather than returning a longer one.
    async fn find_file(&self, id: &ContainerId, dir: &str, name: &str) -> Result<Vec<String>, BackendError>;
...
pub struct ContainerLaunchConfig {
    pub image: Option<String>,          // None = a host task; the image's presence is the mode selector (design #309 §1)
    pub cmd: Vec<String>,
...
**The session transcript is resolved, not computed** (design [#490](design/490-agent-work-on-a-mac.md) D1). The platform hands the agent CLI a `--session-id` and the CLI names the transcript after it, but the *directory* it writes into is the CLI's own slug of the cwd it **resolves** — an external, undocumented, symlink-sensitive name. So the harvest asks `find_file` which files under `{CLAUDE_CONFIG_DIR}/projects` carry `{session_id}.jsonl` and copies the one that resolves; the only external contract relied on is the one the platform itself supplied. The scan is node-local in both deployments — a Docker-endpoint node streams the directory's tar in-process, a worker node scans inside its own daemon — so only the resolved path crosses NATS, and the bytes then ride `copy_file_chunk` (below): a long session's transcript is routinely larger than one reply, so harvesting it with the unchunked `copy_file` silently loses most work-agent transcripts. A resolution that is not exactly one file stores nothing, and is **loud**: zero matches and several matches are both logged at **error** — the agent named a session and the file it must have written is not resolvable, which D1b calls a platform break rather than an empty run. **Every** way a task ends with a session id and no stored transcript leaves a `transcript-missing.json` **marker artifact** on the task, naming which branch fired (`zero`, `several`, a resolution that never answered, or a resolved path the copy did not produce), the session id and the directory searched — so a task with neither a transcript nor a marker means the run named no session at all, and an operator reads the difference off the artifact list rather than off a node's logs. The two non-refusing branches stay at **warn**, except the over-band refusal, which is logged at error and marked `"lost": true` because the platform dropped a record it had already resolved. The marker is the record; **no job's state changes**, because the harvest holds no `&mut Core` and D1b's `transcript_unresolved` escalation stays unarmed until #490's M6 says a legitimate run never produces one. Several matches is never resolved by picking a file: the store keys one transcript per task, so "several" has no representable answer, and any rule for choosing among them would be the guess about a tool's behaviour that D1 removed. `find_file` is **additive** on the worker wire, so it does not bump `WORKER_RPC_VERSION` (§14.1): an N-1 daemon answers the unknown-op error and the caller falls back to the computed path for that node — correct on a container node, which every such daemon is, and never taken for a host task.

**Output archives — one well-known path, no declaration.** A **work-side** container (agent work, command work, wrap-up) may leave an archive at `/workspace/chug-output.tar.gz`; the dispatcher harvests it at exit, before removal, into the `outputs` object store (§1.5) as the task's `output.tar.gz` artifact, served by the same per-task artifact routes as a transcript. `eval-result.json` is the precedent: one path the platform reads from every container of a kind, with nothing declared in the job type — so there is no `outputs:` schema, no config epoch, and no conditional-capture expression language. A script that wants failure-only capture writes the tarball in a shell `trap`; conditionality lives in the work script, where it always did. **Evaluator, merge-gate and triage containers are not read** — an evaluator's structured output already has its own channel (`eval-result.json`, §3.3), and reading the eval fan-out would multiply the disk pressure the `outputs` ceiling is sized against. Absence is the ordinary case and is silent. The archive is capped at the platform's 16 MiB blob ceiling — the same number as a job attachment (§1.6) — and an archive over it is **refused by name and stored not at all**, never truncated, because a partial archive carries nothing where a partial log tail carries its tail. The refusal is logged and the task's own result is unchanged: harvest is reporting, and a job must never fail because its reporting failed. Anything above the band belongs in a cloud bucket, not here. **Of the output harvest's misses, only the over-band refusal is logged at error level**, because it alone names an action its operator can take; every other output-harvest miss — an unreachable node, an N-1 worker that does not yet answer `copy_file_chunk` (§14.1) — is an ordinary warning, since telling a whole node's work containers to move an output to a bucket during a routine worker refresh would send its operator to the wrong action.
...
**The dispatcher-composed stamps (`JOB_*`) are reserved.** `JOB_ID`, `JOB_PROJECT`, `JOB_TYPE`, `JOB_BRANCH`, `JOB_SHA` and `JOB_TASK_ID` are composed by the dispatcher and carry the launch's identity; the `JOB_` prefix is **reserved** for them exactly as `CHUG_` is (§5.3), so declaring a `JOB_`-prefixed **secret or var** in a job type is a release-validation error and injection skips it. Vars and secrets are injected *after* the stamps and would otherwise overwrite them, and `JOB_PROJECT` and `JOB_TYPE` are the whole identity a node can observe about a launch: every node-side grant in the platform — `WORKER_KVM_PROJECTS` (§3.1), `WORKER_NIX_PROJECTS` (§3.1), `WORKER_HOST_PROJECTS` (§3.1) — matches on the first, so a name project config could move is a **grant key project config could move**, and a node-side allow-list would stop being a statement the node alone controls. `JOB_TYPE` is the job type's own `name:` — the same string the workload-identity token's subject carries (§8.3) — and it is stamped so a node-side grant can key on `(project, job type)` rather than the project alone; `WORKER_DOCKER_GRANTS` (§3.1) is the grant that reads it, and it is the one whose key is root-equivalent (design [#517](design/517-docker-access-for-jobs.md) D3). That grant reads one stamp from the *other* sealed prefix as well — `CHUG_PHASE`, the launch's level (§6.3) — because it admits work-level launches alone (design [#543](design/543-placement-granularity.md) D5); the `CHANNEL_ROLE` stamp beside it carries the same distinction and is deliberately **not** what any grant matches, since project config may declare that name. The reservation is those two prefixes and nothing wider: `BASE_BRANCH`, `REPO_URL` and `NATS_URL` are dispatcher-composed too, but no node or platform decision reads them, so shadowing one only misconfigures the shadowing project's own container and they stay declarable.
...
**Workspace bootstrap:** the dispatcher wraps every work and eval container CMD with a standard bootstrap: `WS="${CHUG_WORKSPACE:-/workspace}"; git clone --branch $JOB_BRANCH $REPO_URL "$WS" && cd "$WS" && exec {original CMD}`. Images must provide `git` and an SSH client but do not perform the clone themselves — the platform enforces the `/workspace` contract (command evaluators write `eval-result.json` there; eval agents inspect the diff there).

**`/workspace` and `/chuggernaut` are virtual wire paths, not host paths** (design [#322](design/322-macos-native-runtime.md) §2). Everything above the backend names them literally — the clone destination, an injected file's `container_path`, the `copy_file` argument, `find_file`'s directory argument, and the env *values* that embed one (`GIT_SSH_COMMAND`, `GOOGLE_APPLICATION_CREDENTIALS`, and since #490 slice 5 `CLAUDE_CONFIG_DIR`) — and each backend decides what they address. A container backend takes them literally, so `CHUG_WORKSPACE` is unset and a container task's behaviour is unchanged. The **host** backend (`runtime.mode: host`, §1.1) maps `/workspace/*` → `{task_dir}/workspace/*` and `/chuggernaut/*` → `{task_dir}/chuggernaut/*` and sets `CHUG_WORKSPACE` accordingly. That mapping is **total**: a path outside both prefixes is a hard launch or read error naming the path, never a fall-through to the node's own filesystem, and a wire prefix that **begins a path** in the value of any other environment variable is refused rather than rewritten. A prefix begins a path at the start of a value or after ASCII whitespace, `=`, `:` or a quote; an occurrence continuing a longer segment (`…/kasofsk/chuggernaut.git`, which every job's `REPO_URL` carries here) mentions no wire path and is passed through untouched (job #505). It is total in **both** directions — `find_file` answers in wire paths, so a real path it cannot express as one is refused rather than returned raw (design [#490](design/490-agent-work-on-a-mac.md) D1a). It is what makes a host task possible on macOS at all, where the root filesystem is sealed and `/workspace` cannot be created. A launch's **`cmd` is not one of those surfaces** and never has been, so a command naming a wire path resolves it through the task's own shell — `${CHUG_WORKSPACE:-/workspace}` for the clone (`bootstrap_cmd`), `"$CHUG_HOST_CREDS"` for the agent invocation's prompt, settings and MCP config (§4.3), each unset in a container and therefore unchanged there.

**A `runtime.env` a level inherits is bootstrapped or refused, never skipped.** When the level being launched resolves to one (§1.1 — a level carrying its own `image` under `mode: host` inherits none, and gets the unguarded wrapper), the same wrapper is prefixed with a guard: the task runs only if the node injected `CHUG_ENV_PATH` — the environment root it resolved (§3.1): the store path realised for a `nix:` reference, the selected `Developer/usr` for an `xcode:` one — **and that path is present in the container**, and then runs with `$CHUG_ENV_PATH/bin` at the head of `PATH`. The second half catches a node that injected a path out of a store it did not hand over, or a bundle a task cannot see, which would otherwise put a non-existent directory on `PATH` and pass the guard. A node that dropped the declaration therefore fails the task **loudly, naming the reference**, instead of building against whatever the image happens to carry; that is what makes the wire field additive without the N-1 silent-drop class §14.1 exists to prevent. A job type declaring no env is wrapped byte-identically to before — as is any level that inherits none.
...
An image MAY instead bake a prebuilt `target/` — a *warm-target seed* — and `agent-rust` does not, because the seed is **measured redundant**: against the real `.chug/tasks/ci.sh` command set on air, a seeded build takes 141s and an unseeded one on a warm node cache 186s, for a seed costing 2.26GB in every image, ~600s on every worker-refresh leg (§3.1), and the bulk of that refresh's disk peak. A node cache holding the same workspace costs ~479MB, is shared by all concurrent containers on the node instead of copy-on-written per container, and stays warm between deploys instead of going stale until the next image rebuild. A seed is therefore justified only where the node cache cannot reach — and where it is used, it must not change the build **profile** (a seed compiled differently from what agents run has different fingerprints, and cargo ignores it entirely).
...
**MCP server distribution:** `chuggernaut-channel`, `chuggernaut-ko`, and `chuggernaut-harness` (see §4.5) are built as standalone binaries and shipped alongside the dispatcher. At container launch the dispatcher injects them (as `InjectedFile`s, mode 0755 — see §3.1) into every created container at `/usr/local/bin/chuggernaut-channel` and `/usr/local/bin/chuggernaut-ko` before start. A **host** task is the exception and gets no injected channel binary at all: the dispatcher names the node's own copy instead (§3.1's agent-CLI bullets, design [#490](design/490-agent-work-on-a-mac.md) D2), because the file a node execs is built for that node's userland. The `McpServerConfig.command` field in each `AgentRunConfig.mcp_servers` entry references one of these paths. The binaries connect to NATS using the `NATS_URL` and `NATS_TOKEN` values passed in `McpServerConfig.env`.
...
pub struct AgentRunConfig {
    pub image: Option<String>,              // None = host task; the image's presence is the mode selector, §3.1
    pub prompt: String,
...
**`ClaudeProvider`** — container CMD: `claude -p "$(cat /chuggernaut/prompt.md)" --settings /chuggernaut/agent-settings.json --output-format stream-json --verbose --model {model} --append-system-prompt {system_prompt} --mcp-config /chuggernaut/mcp-config.json`. `--output-format stream-json` (which `-p` requires be paired with `--verbose`) makes the CLI emit JSONL events — assistant text, tool_use, and a final `type:"result"` event — to stdout *as it works*, so the live log viewer (and the harvested `stdout.log` artifact) show intermediate activity rather than silence until exit. The final result event carries the same `usage`/`result`/`session_id` fields the single-object `json` format did, so usage/result harvesting is unchanged; exit-code semantics are unchanged. The serialized `Vec<McpServerConfig>` (`{"mcpServers": …}`) carries the channel server's `NATS_CREDS`, so it is **not** passed inline on argv — inline argv leaks into `ps`, `/proc/*/cmdline`, and crash reports. Instead the dispatcher-composed payload is injected as a mode-0600 file at `/chuggernaut/mcp-config.json` (the CLI accepts a path for `--mcp-config`); credentials travel via injected files or container env, never argv. A **host** task runs the same CMD with its three `/chuggernaut` paths written as `"$CHUG_HOST_CREDS"/…`, since a launch's `cmd` is the one surface §4.1's rebase does not reach; the injected paths themselves stay wire paths, which it does.

**Provider credential delivery (container mode).** The provider's OAuth credential — `CLAUDE_CODE_OAUTH_TOKEN`, which the `global/agents` grant carries (§4.1, §8.2) — is **not** placed in a container launch's environment. It is injected as a mode-0600 file at `/chuggernaut/agent-credential`, and the entrypoint reads that file into a pipe, unlinks it, and runs the bootstrap with the pipe's read end at descriptor 9, naming the number in `CLAUDE_CODE_OAUTH_TOKEN_FILE_DESCRIPTOR`. The reason is not permissions: a process cannot un-publish its own environment, so an env-delivered credential is readable out of `/proc/{pid}/environ` for the process's whole life, while a source read once and closed is readable for a window (design [#529](design/529-secret-handling.md) D3, measured in job #546). This is a **reduction in reach, not a boundary** — the agent CLI runs as the task, so nothing here makes the credential unreadable by the task it was given to. Every such launch also asserts, in the container's own view and printing the verdict to stdout, the two kernel properties the window rests on: Yama's `ptrace_scope` non-zero and `CAP_SYS_PTRACE` absent from `CapEff`. The assertion **reports and never refuses** — an unsatisfied node is no worse off than under env delivery, so failing the launch would cost it all agent work for no gain. `CLAUDE_CODE_REMOTE` is removed from every launch that delivers the credential this way — from the composed env and by an `unset` in the entrypoint, so an image whose own `ENV` sets it is covered too — because with it set the descriptor path persists what it read to a well-known file no cleanup path deletes. A launch still on the env route reads no descriptor, so it keeps the variable and has nothing to persist. A **host** task keeps env delivery: descriptor delivery is a container-launch route only, and the assertion above reads `/proc`, which that path does not have — the equivalent kernel properties there are measured (design [#529](design/529-secret-handling.md) M7: a same-uid process reads another's environment, and `task_for_pid` denies unsigned code the port a memory read needs) but nothing asserts them at launch. `ANTHROPIC_API_KEY` likewise keeps the env route, its descriptor variable being unmeasured. `CHUG_AGENT_CREDENTIAL_FD=0` in the dispatcher's environment reverts every agent launch to env delivery.
...
**Upfront injection (tagged, work containers only) — IMPLEMENTED, repo-versioned form:** job types declare default knowledge entries (`knowledge:`); operators may add more at job creation. An entry naming a markdown file (`docs/reference/style.md`, `docs/implementation-notes.md`) is a **page**, read verbatim from the repo; any other entry is a **tag**, read from `.chug/tags/{tag}.md`. At launch the dispatcher resolves the union — type defaults then job tags, each entry kept once at its first occurrence — from the project repo at `base_ref` (entries without a file are skipped), concatenates them into a `## Project Knowledge` block with one `### {entry}` section each, and injects it via `--append-system-prompt` for Claude (prepended to the prompt for Codex). Attached knowledge is delivered as payload, not as an instruction to go read it: a page is the document itself, so there is no second copy of a rule to drift. Work containers only — eval containers do not receive a pre-injected system prompt. The repo is the primary tag store; the NATS KO buckets described below (global/team scopes, runtime queries via chuggernaut-ko) remain the deferred extension for knowledge that spans projects.
...
The `CHUG_` name prefix is **reserved** for vars exactly as it is for secrets (§5.3): a `CHUG_`-prefixed name in `vars:` is a release-validation error, and injection skips it so a stored one cannot clobber a task-origin stamp (§4.1, §6.3). The `JOB_` prefix is reserved on the same terms and for the same two reasons (§4.1): the dispatcher-composed launch stamps are what a node's grant list matches on.
...
That grant injects the `global/agents` secrets whose names the provider-credential set holds into *every* agent container on the platform, and `admin secret copy --to global/agents` is a one-line operation. Keeping the two mechanisms disjoint — separate declaration, separate bucket, no conversion between them and no `copy` verb — makes that command incapable of expressing the mistake. The residual is named rather than hidden and design [#529](design/529-secret-handling.md) S1b narrowed it without closing it: a copied service-account key now reaches no container unless the operator also *names* it as a provider credential, which nothing mechanical can prevent — a name is all the platform can read. What this guarantees is that the *supported* mechanism has no path there.
...
A project's markdown documentation is a first-class output produced by agent jobs, gated through the normal merge path like any code change. Three job types produce documentation, split because their review criteria differ:

- **`design`** — architecture/plan documents that argue a decision's tradeoffs and set direction, written to `docs/design/{seq}-{slug}.md` — the leading `{seq}` is the filing job's number, and the shape is enforced by the stage-1 lint below because the Designs view joins a design to its jobs on that filename. Typical flow: a `design` job lands the document, then `code`/`web` jobs depend on it and cite it. The reviewer judges whether the document addresses the brief, weighs its alternatives and tradeoffs honestly, and stays consistent with `docs/spec.md` and the codebase as they exist.
...
- **`molt`** — the periodic shedding of the corpus at a milestone: compacting mutable heads, deleting fully-implemented design documents outright, and repointing every referrer. It is the inverse of the other two, and that inversion is the reason it is its own type. A `design` or `docs` job removes sentences that became **false** or adds ones that were **missing**; a molt removes sentences that are still **true**, because the saga of how a decision was reached stops being worth its reading cost once the decision is simply how the system works. So its evaluators cannot ask "is this accurate" — they ask whether the shedding was licensed and whether the books balance: a command evaluator checks the accounting (a vanished landed-slice claim, a surviving document that lost its last referrer, a deletion that was not eligible, a deleted path still cited from code with no stub, a shed with no ledger line), and a second agent evaluator is instructed to **refute** — to name a fact the base stated that the result does not, and the wrong action its absence causes. It then ends in a **human evaluator** — the only shipped job type that does. That is not ceremony: the failure a molt most likely dies of is a shed rejected alternative, which names no path, constant or link, has no signature in a diff of legitimate deletions, and surfaces only when someone re-proposes the rejected thing with no argument to hand. No gate can see it and an adversary can only refute what it thought to look for, so the last reader is a person who knows what the project is about to do next — and the work agent's summary is written for that reader, close calls first, rather than as a changelog. A molt is scoped by its brief and takes the project quiescent; both its work agent and its loss evaluator may be granted `tools:` (§1.1) because the reading cost is the whole corpus and it parallelises.
...
**Gating.** All three types stage their evaluators (§3.3): stage 0 is the agent reviewer described above — joined, for `molt`, by the accounting command evaluator, so the cheap mechanical refusal happens before anything slower; stage 1 is a shared documentation lint (`.chug/tasks/doc-lint.sh` — markdown well-formedness, intra-repo link resolution, and the `docs/design/{seq}-{slug}.md` filename shape) that runs alongside the appended project `ci` default (§1.1). Both stage-1 checks are diff-aware and self-skip a diff with no relevant files, so a doc-only change is gated in seconds — `ci` skips its build because no `crates/**` path changed, and `doc-lint` runs only over the changed `.md` files. What a doc *claims about the tree* is gated separately and for every job type, not just these three: the `ci` default's pure-shell pre-stage resolves every factual claim a tracked `*.md` makes about this tree against git — backticked paths, restated constants, landed-slice rows, owned definitions, the catalogue, and the heading a link's `#anchor` names (`.chug/tasks/check-doc-facts.sh`) — because the claims are made by every job type and a `docs`-scoped check misses most of the drift.
...
- No privileged mode
- No host network
- No host volume mounts **by the dispatcher** — the §3.1 closed class of worker-provisioned node properties is the exception, and the docker socket is the member that is a capability rather than an accelerator: a node may bind it into an allow-listed `(project, job type)` and that launch then holds node root (design [#517](design/517-docker-access-for-jobs.md) D1). Nothing is bound anywhere until a node's own config names the pair
...
forge, repo-versioned by design — CLAUDE.md). Auto-deploy also gives
...
On 2026-07-22 they were not: a `wrap_up` section merged into
`.chug/jobs/web.yaml`, the running dispatcher's strict parser rejected the
unknown key, and every `web` job escalated at launch
(`launch_validation_failed`) until an operator deployed. Merging config had
silently become deploying config. This part defines the rules that make that a
*detected, explained, non-destructive* condition.

...
# platform agent credentials (agent containers only): every secret in the reserved global/agents scope, env-named by the secret; declared secrets win on collision. EXCEPT the provider's OAuth credential, which a container launch delivers on an inherited file descriptor instead of an env name (§4.3)
GITHUB_TOKEN    ...

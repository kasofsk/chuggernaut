...
| 4 | `docs` — a provisioning runbook (create the user, the group **as the user's primary group** per D12, the home, the `sudoers` line and **the `WORKER_HOST_ROOT` root itself**; verify; decommission; and the deletion asymmetry); `docs/reference/runbooks/worker-host-projects.md` §2 stops arguing single-tenancy as the boundary | runbook set | 3, 8 | **Landed** (job #572). `docs/reference/runbooks/worker-host-users.md` is the procedure — the ordering that slice 8 has to precede it, the root, the group as the **primary** group, the `sudoers` line, what to run to prove the binding rather than discover it at the next host job, and the decommission whose account-record half D9 says cannot be done over ssh. The sibling keeps its single-tenancy argument and gains what it is still for: the roster, and `WORKER_NIX_PROJECTS` as the one thing a uid does not bound. **Nothing is provisioned** — the runbook is written against a fleet where the escalation has never run with a real project user |

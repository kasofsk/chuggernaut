...
The `ContainerBackend` implementations drive the container runtime directly. The workload demands only five operations (launch, wait, kill, inspect, copy-file), and the dispatcher is already the scheduler — so the v1 production default is a **fleet of plain Docker daemons**: one node or several, each just an endpoint with a slot cap, platform services under docker compose on one of them. No Swarm, no cluster state — placement intelligence lives in the dispatcher. The Kubernetes Jobs backend (k3s self-hosted, or managed EKS/GKE/AKS) exists for consumers who outgrow a small fleet. Workflow engines and CI systems (Argo, Tekton, Temporal, forge Actions) are deliberately excluded at any scale: they bundle their own DAG, state store, and retry semantics — exactly the layer the dispatcher owns — and would reintroduce the reconcile-against-a-second-writer problem that removing the git forge eliminated.

### Fast iteration inside the work container; independent evaluation as the gate

The v1 lesson: author↔reviewer exchanges as separate dispatched runs (forge Actions workflows) cost a full checkout and agent cold start per round. v2 splits the loop in two. The **inner loop** runs inside the work container: a job type may declare `work.review`, and an injected harness alternates the author agent with a reviewer agent (fresh session each round, configurable provider/model — Claude only in v1) until acceptance or the iteration budget (default 5) runs out. Same working tree, same build caches, the author resumes its session across rounds — iteration costs seconds, not a container relaunch. Each round is reported as a **step** (job → task → step granularity), so the tracker UI shows the live ping-pong without steps ever becoming pipeline definitions in YAML. The **outer loop** — the Evaluation phase — is unchanged and authoritative: the inline reviewer shares the work container's credentials and is therefore tamperable by the author, so its acceptance is advisory by construction. CI and the independent evaluators only run once per cycle, after acceptance.

### Evergreen default branch via a merge gate

Evaluation runs against the job branch built on its pinned `base_ref`; by the time eval passes, other jobs may have landed. A textually clean squash-merge can still be semantically broken. So before merging, the dispatcher checks whether the default HEAD moved — if it did, it builds the candidate squash commit and re-runs the required command evaluators (the test suite) against it, serialized as a depth-1 merge queue. Gate failure re-enters Work on the new base without consuming rework budget, mirroring the merge-conflict path. Nothing reaches the default branch without the tests passing against the exact tree that lands; jobs with an unmoved base skip the gate entirely. Project-wide `.chug/jobs/_defaults.yaml` evaluators make the evergreen gate a project property rather than a per-job-type convention.

...
Status: DORMANT as a plan; its analysis is live again for the coming reimplementation.

**The phases are not scheduled work; the analysis they carry is current.**
[#215](215-refactor-plan.md) reversed the language decision argued here — stay in
Rust, refactor incrementally — and that reversal is itself now spent: the project
is beginning a **fresh reimplementation of the dispatcher, language not yet
chosen**. So the four ideas this page was kept for stop being a legacy the
reversal preserved and become directly applicable:

- **golden decision traces** — `{initial state, event, expected transitions,
  expected effects}` as data, generated by instrumenting a running dispatcher
  rather than written by hand;
- **effects-as-data** — the decider returns a list of effects and performs none;
- the **wire-level conformance suite**, and shadow-mode decision diffing against
  a live implementation;
- the **contract-first change rule** — a change names the contract it moves, and
  if that contract does not exist yet, writing it is the first commit.

Read the phases below for those. What does *not* transfer is everything that
presupposes TypeScript specifically, or a live Rust dispatcher to shadow: the
`ts/` workspace layout, and the shadow/cutover sequencing.

**The language argument below is live, not settled.** Because the
reimplementation has not picked a language, the "Decision: TypeScript (over
Python)" rationale and the #208 Python proposal it displaced are both back on the
table, and are recorded here rather than re-derived. Read them as candidates
weighed once, not as a decision taken.

There is no slice table: a dormant plan's phases are not slices, and giving them
landed states would imply a queue that does not exist.

The blockquote below is the original supersession note, left as written. It is
superseded in turn by the paragraph above — its instruction not to action the
language migration reflects #215's reversal, not the current position.

...
these types are the compiled-against definition on both sides of the wire
...
- Typed accessors per bucket: `JobStore`, `TaskStore`, `ProjectStore`, `CounterStore`, `RdepsStore`, `StepStore`, `ArtifactStore`
...
- **Node properties** a worker sets on its own backend and the dispatcher never does (§3.1): the node-local build cache bind, the `/dev/kvm` passthrough with its read-only toolchain mounts (design #367 A1), the Xcodes a host-capable node discovered at boot, which a launch's `xcode:<version>` resolves against (`crates/worker/src/xcode.rs`, design #322 W4), the agent CLI the same node discovers on the daemon's own `PATH` (`crates/worker/src/agent_cli.rs`, design #490 D3), and the per-project unix users a host node resolves out of its own passwd database (`crates/worker/src/host_users.rs`, design #537 D3). None of them reaches the wire or `ContainerLaunchConfig` — each discovered fact is advertised on `NodeCapabilities`, which is a report **except where a matcher reads it**: `choose_placement` takes exactly three of its fields (`modes`, `resources_enforced` and, since design #543 S1, `envs`), so the Xcodes a node discovered are a placement input and not only a diagnostic, while `platform`, `agent_cli`, `docker_reachable` and `leases` reach no scheduler at all
...
guarantees / spec §); `docs/reference/modules.md` is the one-line registry.
...
platform-ops, is its own crate at `crates/platform-ops` (refactor-plan C9);
`platform_ops.rs` here is its adapter.
...
dispatcher/
  ...
  platform_ops.rs— the adapter for the platform-ops context, which is its own crate
...
- **Host preparation** — `nix/`: the `chug-node` NixOS and nix-darwin modules a worker node's host repo imports (`flake.nix` exports them as `nixosModules.chug-node` / `darwinModules.chug-node`, design #372). Nix, not Rust; **nothing in CI evaluates it** (#372 §2.3) — the consuming host repo's `nixos-rebuild build` is the gate. It also declares the worker daemon's systemd unit (design #440 slice 7), from a template `nix/chug-node/chug-worker-unit.test.sh` pins against the one `deploy/prod/build-worker.sh` renders; that suite is text over text and evaluates no nix either. **Three clocks own a
  node's environment and never mix** (#372 §7): the *system closure* changes by
  operator drain-and-rebuild and is what `nix/` is; the *worker daemon* is the
  platform's and changes through `deploy/prod/worker-refresh.sh`; the *task
  environment* is the project repo's and changes by `git push` with no deploy.
  The operational test for whether something belongs in the module is **if two
  projects on the same node could reasonably want different values for it, it
  does not** — and that applies to an option's *value*, not only its name, so a
  key that looks like a machine fact but takes a per-project derivation as its
  value fails it. The same line is why the module may declare machine facts and
  may never declare platform state such as a node's name or its slot count.

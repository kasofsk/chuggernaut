...
- **Node properties** a worker sets on its own backend and the dispatcher never does (§3.1): the node-local build cache bind, the `/dev/kvm` passthrough with its read-only toolchain mounts (design #367 A1), the Xcodes a host-capable node discovered at boot, which a launch's `xcode:<version>` resolves against (`crates/worker/src/xcode.rs`, design #322 W4), and the agent CLI the same node discovers on the daemon's own `PATH` (`crates/worker/src/agent_cli.rs`, design #490 D3). None of them reaches the wire or `ContainerLaunchConfig` — each discovered fact is advertised on `NodeCapabilities`, which is a report rather than an input

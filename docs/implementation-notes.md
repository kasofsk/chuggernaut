...
- **fn build_host_config** — The KVM device and the two read-only toolchain mounts are decided once, from `KvmGrant::admits`, so an edit cannot hand out the wider of the two on its own (design #367 A1). The `debug_assert` pins that pairing by counting read-only mounts rather than testing `mounts.is_some()`, because the cache mount is a third, independent one: a cache with no grant and a grant with no cache are both legal, and only the toolchain pair is all-or-nothing.
...
- **fn inject_android_env** — Same allow-list decision as the mounts, through the same `KvmGrant::admits`: env that pointed at an SDK the launch did not get would fail deep inside the task instead of at the launch.

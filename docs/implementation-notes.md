...
- **fn remove** — Deleting the shared workspace needs an ownership record, which #309 §2(c) does not mention: under option (iii) the workspace is one fixed path, so a `remove` arriving after the next launch has claimed it would delete the *new* task's clone. The `workspace-owner` file is a file rather than a field because it has to survive a daemon restart. Option (i)'s per-task rebase dissolves this entirely.
- **fn remove** — Also reclaims the paths the launch materialized outside the task directory (`/chuggernaut/prompt.md`, the ssh cert, the MCP config), which `meta.json` records at launch. Nothing else on the node reclaims them, so a failure is both an `Err` and an `error!`.
...
- **fn launch** — `resources.cpu`/`memory` warn and run unbounded. #309 §7 wants a capability and a hard refusal (P2/P3), but every job type in `.chug/jobs/` declares both, so refusing in P0 would leave nothing to prototype against; `task_timeout` still bounds the task in time.
...
- **fn validate_runtime** — `mode: host` is refused as `Unsupported`, not `Invalid`: the declaration is the one design #309 §3 specifies, and what is missing is the backend (P0) and the placement (P1), so the message names the phase rather than blaming the author. Refusing here is also what keeps a host job type from reaching placement and queueing 30 minutes for a node that cannot exist.
...
- **fn validate_runtime** — The host row's own field rules (top-level `image` disallowed, `runtime.env` required) are deliberately absent: stating them requires narrowing the evaluator-image requirement too, which design #309 assigns to P1, and a refused mode gains nothing from rules about what it would have required. `RUNTIME_SCHEMA_EPOCH` is frozen per-feature beside `INPUTS_SCHEMA_EPOCH` for the reason spec §14.2 gives — a later bump for an unrelated feature must not raise what an existing `runtime:` config declares.
...
- **fn local_backend** — Single-node backend named after this node so returned container ids are already `{node}/{docker_id}` — the fleet backend routes on that prefix.
- **fn local_backend** — Node-local build cache: a worker-side property, added here from the worker's own config — never from the launch message (spec §3.1). Its `create_dir_all` only reaches the host path when the daemon runs natively on the node; the shipped daemon is containerized without that path mounted (design #372 C3), so on a node the host dir is a provisioning prerequisite and since #379 a missing one fails every launch. Concurrent containers share it, which sccache handles by locking.
- **fn local_backend** — `with_cache_dir` and `ping_all` are inherent to `DockerBackend`, so the wiring has to finish before the backend is erased behind `Arc<dyn ContainerBackend>` (design #322 W1). Keeping it in one function is what makes a second runtime a construction-time choice rather than a branch in every op handler.
- **fn local_backend** — A declared KVM device that is absent refuses the boot, the same shape an unserviceable `WORKER_MODES` entry gets: a node advertising a capability it cannot serve would otherwise fail every allow-listed launch at container create, one job at a time. The daemon runs in its own container (`deploy/prod/build-worker.sh`), so this check reads the daemon container's view — enabling `WORKER_KVM` on a deployed node means passing the device into the daemon container too, and design #367 correction 12 is the record of why that is a precondition rather than an assumption.
- **fn local_backend** — P0 of design #309 has no per-request mode selector — routing is `placement.node` plus `WORKER_MODES` — so a node that names `host` at all serves *every* launch as a host process and ignores the declared `image`. #309's Phasing table calls that a deliberate lie that must never leave the prototype node, which is why the boot warning says so rather than leaving it to the design doc.
...
- **fn enforce_host_capacity** — #309 §2 and §5 both describe `slots: 1` as configuration, and configuration is not enforcement: an operator's `WORKER_SLOTS=4`, or a runtime `set_slots` raise (spec §3.1 operator capacity control, which the design never reconciles with the 1-slot pin), would put two host tasks on one `/workspace`. Refusing the boot when either value is not 1 is the half that cannot be argued with; `HostBackend::admit` is the runtime half.

...
- **fn probe_supervision** — It runs `systemd-run` under `env_clear` plus `floor_env`, the same environment `spawn_task` gives the launcher, because the two diverging is exactly the failure the probe exists to prevent: a probe that inherited the daemon's `XDG_RUNTIME_DIR` or `DBUS_SESSION_BUS_ADDRESS` would find a user bus at boot that no launch can reach, and the node would advertise `host` and then fail every task. The composed environment carries neither variable by design (#440 slice 1), so a user scope is reachable only through `sd-bus`'s `/run/user/$UID/bus` fallback — measured working on `gumbo-nuc-0` with `XDG_RUNTIME_DIR` unset, and a refusal naming `loginctl enable-linger` when the uid has no running user manager.

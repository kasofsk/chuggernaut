...
- **fn build_host_config** — The KVM device and the two read-only toolchain mounts are decided once, from `KvmGrant::admits`, so an edit cannot hand out the wider of the two on its own (design #367 A1). The `debug_assert` is that pairing.
- **fn read_only_bind** — `HostConfig.mounts` rather than a `binds` string, because the engine REFUSES a bind whose source is missing while `-v` silently creates it as an empty root-owned directory — measured both ways on `gumbo-nuc-0`, 2026-08-02. The existing sccache bind still goes through `binds` and carries that hazard; migrating it is a behavior change to a shipped path and belongs in its own ticket.
...
- **fn build_backend** — Node-local build cache: a worker-side property, added here from the worker's own config — never from the launch message (spec §3.1). The daemon owns creating/owning the host dir; concurrent containers share it, which sccache handles by locking.

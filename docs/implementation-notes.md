...
- **fn inject_toolchain_env** — Same allow-list decision as the mounts, through the same `KvmGrant::admits`: env that pointed at an SDK the launch did not get would fail deep inside the task instead of at the launch. `FLUTTER_ROOT` is injected only when the node declares `WORKER_FLUTTER_DIR`, for that same reason, and the function is named for the toolchains rather than for Android since #393 gave it a second, non-Android one.

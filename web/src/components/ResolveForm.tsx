...
export function ResolveForm({
  ...
  preWork?: boolean;
  /** human evaluator task: failing offers the abort verdict (docs/reference/design-lifecycle.md) */
  evaluator?: boolean;
  ...
  return (
    <div className="resolve">
      <input
        placeholder={
          approval
            ? "notes (optional; required to reject)"
            : "notes (optional; required to fail)"
        }
        value={notes}
      ...
      {escalation ? (
        <>
          <button
            onClick={() =>
              onResolve({ kind: "Escalation", action: "Retry", structured })
            }
          >
            retry
          </button>
          {!preWork && (
            <button
              onClick={() =>
                onResolve({ kind: "Escalation", action: "Resolve", structured })
              }
            >
              resolve
            </button>
          ...
          <button
            className="danger"
            onClick={() =>
              onResolve({ kind: "Escalation", action: "Revoke", structured })
            }
          >
            revoke
          </button>
        ...
        <>
            ...
            title={
              structured
                ...
                : approval
                  ...
                  : "failing requires notes"
            }
            onClick={() =>
              structured &&
              onResolve({ kind: "Fail", structured, abort: abort || undefined })
            }
          >
          ...
          {evaluator && (
            <label
              className="dim"
              title="not satisfiable by rework — skip retries and escalate to a human"
            >
              <input
                type="checkbox"
                checked={abort}
                onChange={(e) => setAbort(e.target.checked)}
              />{" "}
              abort (unfixable)
            </label>
          )}

...
describe("docLinkTarget", () => {
  it("resolves a sibling document against the doc directory", () => {
    ...
    expect(target("311-job-inputs.md")).toBe(
      `${base}?path=docs%2Fdesign%2F311-job-inputs.md`,
    );
  });

  it("walks `..` up to the repo root", () => {
    expect(target("../../CLAUDE.md")).toBe(`${base}?path=CLAUDE.md`);
    expect(target("../adr/0001-nats.md")).toBe(
      `${base}?path=docs%2Fadr%2F0001-nats.md`,
    );
  });
  ...
  it("resolves against the repo root for a document at the top level", () => {
    expect(docLinkTarget(base, "README.md", "./CLAUDE.md")).toBe(
      `${base}?path=CLAUDE.md`,
    );
  });
});
